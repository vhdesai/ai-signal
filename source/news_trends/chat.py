...
def _chat_shell(mode: str, title: str, subtitle: str, compact: bool = False) -> str:
    ...
    footnote = (
        "Responses stream from the configured Cloudflare Worker and may include article-backed context."
    )
    return f"""
<div class="ai-chat-app ai-chat-app--{mode}" data-ai-chat-root data-chat-mode="{mode}">
  ...
  <div class="ai-chat-footnote">{footnote if not compact else 'Compact AI Signal chat powered by article-aware context injection.'}</div>
</div>
"""
...
(function(){
  ...
  const ARTICLES_URL = '{{ rel }}articles.json';
  const OPEN_FULL_CHAT_URL = '{{ rel }}chat.html';
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        await streamResponse({
          ...
          assistantText += token;
          typing.bubble.innerHTML = escapeHtml(assistantText).replace(/\n/g,'<br>');
          messagesEl.scrollTop = messagesEl.scrollHeight;
        });
  ...
  <div class="ai-chat-panel-shell">
    <div class="ai-chat-panel-topbar">
      ...
      <div class="ai-chat-panel-actions">
        <a class="ai-chat-open-full" data-chat-open-full href="{{ rel }}chat.html">Open full chat</a>
...
<section class="chat-page-shell">
  <div class="chat-page-intro">
    <div>
      <h2>Explore AI news with a context-aware assistant</h2>
      <p>Ask about companies, regulation, infrastructure, or events. The assistant loads <code>articles.json</code>, finds the most relevant AI Signal stories, injects the top matches into the system prompt, and streams answers from your selected free model.</p>
    </div>
    <div class="chat-page-chips">
      <span class="chat-page-chip">RAG from articles.json</span>
      <span class="chat-page-chip">Streaming SSE responses</span>
      <span class="chat-page-chip">Free OpenRouter models</span>
    </div>
  </div>
"""
        + _chat_shell("page", "AI Signal assistant", "Article-aware chat for AI industry news and trend exploration.")
        + """
</section>
"""
    )
