---
article_id: 2026-07-23-runway-launches-model-router-for-generative-media-workflows
title: Runway launches model router for generative media workflows
date: '2026-07-23'
source: TechCrunch
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-07-25-daily-ai-news-digest.md
content_hash: b602b1bcecebb94b5b3a34a7a23cd1cf611b66f52a089436f18d1559ba76a2fc
normalized_title_hash: 24d64af9885921b9
canonical_url_hash: ''
tags:
- Runway
- Model routing
- Generative media
entities: []
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-27-openrouter-cements-its-position-as-the-dominant-multi-model
- 2026-05-27-capitalg-s-lead-on-openrouter-signals-alphabet-s-distributio
- 2026-07-22-cursor-releases-cursor-router-a-request-level-classifier-for
- 2026-07-07-microsoft-joins-ai-cost-cutting-trend-by-relying-more-on-its
embedding_id: 2026-07-23-runway-launches-model-router-for-generative-media-workflows
event_name: ''
---

# Runway launches model router for generative media workflows

Runway launched Media Router, a developer tool that automatically chooses image, video, or audio generation models based on quality, speed, or cost preferences. The company is positioning itself as infrastructure for generative media rather than only a model provider, with customers able to access Runway and third-party models through API workflows. The move mirrors LLM model routing and suggests media generation will become a multi-model orchestration problem. URL: https://techcrunch.com/2026/07/23/runway-bets-on-ai-model-routing-as-generative-media-gets-crowded/

<!-- graph:start -->
## Connections

**Topics:** [[Corporate Moves]]
**Related:** [[2026-05-27-openrouter-cements-its-position-as-the-dominant-multi-model]] · [[2026-05-27-capitalg-s-lead-on-openrouter-signals-alphabet-s-distributio]] · [[2026-07-22-cursor-releases-cursor-router-a-request-level-classifier-for]] · [[2026-07-07-microsoft-joins-ai-cost-cutting-trend-by-relying-more-on-its]]
<!-- graph:end -->
