---
article_id: 2026-07-13-google-s-waze-rolls-out-gemini-powered-features-motorcycle-m
title: 'Google''s Waze rolls out Gemini‑powered features: Motorcycle mode and "Less
  Chatty" navigation'
date: '2026-07-13'
source: Engadget
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-07-13_060930_Inbox_Daily AI News Digest - July 13,
  2026.md
content_hash: 3beb80f790d2dc58bbcdb655dd4d6dbbf03cc5dc64fe697904e906f12a2a0556
normalized_title_hash: 746b54a2d6dff7dc
canonical_url_hash: ''
tags: []
entities:
- Google
themes:
- datacenter-infrastructure
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-13-waze-rolls-out-new-gemini-powered-ai-features-including-moto
- 2026-07-13-waze-adds-gemini-powered-conversational-search-and-route-per
- 2026-07-13-google-pushes-gemini-deeper-into-chrome-waze-and-india-s-ent
- 2026-05-20-google-unveils-gemini-omni-and-new-consumer-agent-features
embedding_id: 2026-07-13-google-s-waze-rolls-out-gemini-powered-features-motorcycle-m
event_name: ''
---

# Google's Waze rolls out Gemini‑powered features: Motorcycle mode and "Less Chatty" navigation

Waze introduced an AI‑built Motorcycle mode that accounts for rider‑specific shortcuts and hazards (potholes, speed bumps, narrow bridges), a "Less Chatty" voice option, and route suggestions based on prior trips — part of a broader push to embed Gemini across Google's navigation apps. The features still lean on Waze's real‑time traffic map and human, motorcycle‑focused editors for accuracy. It's a modest but concrete example of Gemini moving into Google's consumer surfaces.

<!-- graph:start -->
## Connections

**Entities:** [[Google]]
**Topics:** [[Infrastructure & Compute]]
**Related:** [[2026-07-13-waze-rolls-out-new-gemini-powered-ai-features-including-moto]] · [[2026-07-13-waze-adds-gemini-powered-conversational-search-and-route-per]] · [[2026-07-13-google-pushes-gemini-deeper-into-chrome-waze-and-india-s-ent]] · [[2026-05-20-google-unveils-gemini-omni-and-new-consumer-agent-features]]
<!-- graph:end -->
