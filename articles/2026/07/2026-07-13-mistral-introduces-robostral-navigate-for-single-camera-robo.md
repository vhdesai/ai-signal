---
article_id: 2026-07-13-mistral-introduces-robostral-navigate-for-single-camera-robo
title: Mistral introduces Robostral Navigate for single-camera robot navigation
date: '2026-07-13'
source: Mistral AI
url_original: https://mistral.ai/news/robostral-navigate/
url_canonical: https://mistral.ai/news/robostral-navigate/
url_status: found
digest_source: digests\raw\2026-07-14_082213_Inbox_Fw Daily AI News Digest - July
  14, 2026.md
content_hash: 177d5ff6b2844f8078e8e9bb0f7c7460a1efe3821d9c374675a8af05449e9fb6
normalized_title_hash: 47797ad1f053cb29
canonical_url_hash: 9588f0d947e93e5f
tags:
- New
entities:
- Mistral
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-14-mistral-releases-robostral-navigate-for-single-camera-embodi
- 2026-07-14-mistral-ai-releases-robostral-navigate-an-8b-model-for-robot
- 2026-07-15-mistral-releases-robostral-navigate-for-single-camera-robot
- 2026-07-09-mistral-enters-physical-ai-with-robostral-navigate
- 2026-05-10-mistral-medium-3-5-128b-enterprise-open-weight-model-with-re
embedding_id: 2026-07-13-mistral-introduces-robostral-navigate-for-single-camera-robo
event_name: ''
---

# Mistral introduces Robostral Navigate for single-camera robot navigation

Mistral released an 8B embodied-navigation model that uses a single RGB camera and natural-language instructions to move robots through real environments. The company says it reaches 76.6% success on R2R-CE validation unseen and was trained entirely in simulation across roughly 400,000 trajectories.

<!-- graph:start -->
## Connections

**Entities:** [[Mistral]]
**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-07-14-mistral-releases-robostral-navigate-for-single-camera-embodi]] · [[2026-07-14-mistral-ai-releases-robostral-navigate-an-8b-model-for-robot]] · [[2026-07-15-mistral-releases-robostral-navigate-for-single-camera-robot]] · [[2026-07-09-mistral-enters-physical-ai-with-robostral-navigate]] · [[2026-05-10-mistral-medium-3-5-128b-enterprise-open-weight-model-with-re]]
<!-- graph:end -->
