---
article_id: 2026-07-05-llamaindex-published-legal-kb-a-public-reference-application
title: LlamaIndex published legal-kb, a public reference application that gives AI
  agents filesystem-style access to a…
date: '2026-07-05'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-07-05_061041_Inbox_Daily AI News Digest - July 5,
  2026.md
content_hash: 88f04e1c0922c5e9e6309ebbcc560678a82ba5ee39c3641c1b1fe2e49ff721d1
normalized_title_hash: 83d6dc17f432998a
canonical_url_hash: ''
tags: []
entities: []
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-05-llamaindex-ships-legal-kb-an-agentic-retrieval-reference-app
- 2026-05-23-hkust-paper-llm-judge-agents-commit-serious-legal-errors-in
- 2026-05-10-new-arxiv-may-2026-1-200-ai-papers-agentic-reputation-system
- 2026-08-06-openai-launches-codex-security-in-research-preview
- 2026-05-22-six-peer-reviewed-springer-papers-published-legal-ai-agents
embedding_id: 2026-07-05-llamaindex-published-legal-kb-a-public-reference-application
event_name: ''
---

# LlamaIndex published legal-kb, a public reference application that gives AI agents filesystem-style access to a…

LlamaIndex published legal-kb, a public reference application that gives AI agents filesystem-style access to a document knowledge base built on Index v2. It exposes retrieve (hybrid semantic search), find, read, and grep as agent tools, with per-file versioning and visual citations. It is a compact but useful template for teams building document-grounded agents.

<!-- graph:start -->
## Connections

**Topics:** [[Corporate Moves]]
**Related:** [[2026-07-05-llamaindex-ships-legal-kb-an-agentic-retrieval-reference-app]] · [[2026-05-23-hkust-paper-llm-judge-agents-commit-serious-legal-errors-in]] · [[2026-05-10-new-arxiv-may-2026-1-200-ai-papers-agentic-reputation-system]] · [[2026-08-06-openai-launches-codex-security-in-research-preview]] · [[2026-05-22-six-peer-reviewed-springer-papers-published-legal-ai-agents]]
<!-- graph:end -->
