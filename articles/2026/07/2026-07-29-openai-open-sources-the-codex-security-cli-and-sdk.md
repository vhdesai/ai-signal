---
article_id: 2026-07-29-openai-open-sources-the-codex-security-cli-and-sdk
title: OpenAI open-sources the Codex Security CLI and SDK
date: '2026-07-29'
source: OpenAI Developer Community / GitHub
url_original: https://community.openai.com/t/introducing-the-open-source-codex-security-cli/1388319
url_canonical: https://community.openai.com/t/introducing-the-open-source-codex-security-cli/1388319
url_status: found
digest_source: digests\raw\2026-07-29_061307_Inbox_Daily AI News Digest - July 29,
  2026.md
content_hash: 43ec3be0d8882058a441dc19b9740e6f6b9f35981bfa50979c1f88cd26049dea
normalized_title_hash: 4d032a8b63571800
canonical_url_hash: 74425c000cb5c7ac
tags: []
entities:
- OpenAI
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-04-20-openai-codex-0-122-0-ships-broad-platform-update
- 2026-05-22-openai-released-codex-version-0-133-0-today-enabling-goals-b
- 2026-05-27-openai-ships-codex-0-134-0-with-hardened-mcp-and-cli-profile
- 2026-05-13-openai-publishes-codex-windows-sandbox-architecture-and-tans
- 2026-05-13-openai-codex-now-available-from-anywhere-windows-sandbox-lau
embedding_id: 2026-07-29-openai-open-sources-the-codex-security-cli-and-sdk
event_name: ''
---

# OpenAI open-sources the Codex Security CLI and SDK

OpenAI released an open-source Codex Security CLI and TypeScript SDK under Apache-2.0 (install via npm i @openai/codex-security). The tool scans repositories, validates vulnerabilities, generates patches, tracks findings, and adds security gates to CI/CD pipelines. OpenAI described it as an early beta and is actively soliciting developer feedback.

<!-- graph:start -->
## Connections

**Entities:** [[OpenAI]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-04-20-openai-codex-0-122-0-ships-broad-platform-update]] · [[2026-05-22-openai-released-codex-version-0-133-0-today-enabling-goals-b]] · [[2026-05-27-openai-ships-codex-0-134-0-with-hardened-mcp-and-cli-profile]] · [[2026-05-13-openai-publishes-codex-windows-sandbox-architecture-and-tans]] · [[2026-05-13-openai-codex-now-available-from-anywhere-windows-sandbox-lau]]
<!-- graph:end -->
