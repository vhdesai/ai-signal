---
article_id: 2026-07-27-nvidia-microsoft-spacex-palantir-and-ibm-launch-an-open-secu
title: Nvidia, Microsoft, SpaceX, Palantir and IBM launch an “Open Secure AI Alliance”
  — without OpenAI, Google or Anthropic
date: '2026-07-27'
source: CNBC
url_original: https://www.cnbc.com/2026/07/27/nvidia-ai-initiative-openai-cyber-attack.html
url_canonical: https://www.cnbc.com/2026/07/27/nvidia-ai-initiative-openai-cyber-attack.html
url_status: found
digest_source: digests\raw\2026-07-27_061009_Inbox_Daily AI News Digest - July 27,
  2026.md
content_hash: 1c22a10e3483fc5e51a0dba6fa57d0060bd11dbbc7ece2b6ede7fa0f83641236
normalized_title_hash: 82ed2ce0c4a469c7
canonical_url_hash: 64eeef18cd78cd17
tags: []
entities:
- Anthropic
- Google
- IBM
- Microsoft
- NVIDIA
- OpenAI
- Palantir
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-07-27-nvidia-launches-open-secure-ai-alliance-without-openai-googl
related_article_ids:
- 2026-07-27-nvidia-spacex-microsoft-palantir-launch-open-secure-ai-allia
embedding_id: 2026-07-27-nvidia-microsoft-spacex-palantir-and-ibm-launch-an-open-secu
event_name: ''
---

# Nvidia, Microsoft, SpaceX, Palantir and IBM launch an “Open Secure AI Alliance” — without OpenAI, Google or Anthropic

Nvidia, joined by Microsoft, SpaceX, Palantir, IBM and roughly three dozen companies and open-source foundations, launched the Open Secure AI Alliance to give cyber defenders access to frontier models and shared tooling, per CNBC. Notably absent are OpenAI, Google and Anthropic — an omission CSO Online highlighted and that underscores competitive fault lines in AI security. The alliance is explicitly framed as a response to the fallout from the first documented autonomous-agent cyber incident.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Google]] · [[IBM]] · [[Microsoft]] · [[NVIDIA]] · [[OpenAI]] · [[Palantir]]
**Topics:** [[Policy & Regulation]]
**Canonical:** [[2026-07-27-nvidia-launches-open-secure-ai-alliance-without-openai-googl]]
**Related:** [[2026-07-27-nvidia-spacex-microsoft-palantir-launch-open-secure-ai-allia]]
<!-- graph:end -->
