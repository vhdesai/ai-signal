---
article_id: 2026-07-25-28-9m-parameter-llm-now-runs-locally-on-an-8-esp32-microcont
title: 28.9M-Parameter LLM Now Runs Locally on an $8 ESP32 Microcontroller
date: '2026-07-25'
source: GitHub**
url_original: https://github.com/slvDev/esp32-ai
url_canonical: https://github.com/slvDev/esp32-ai
url_status: ok
digest_source: digests\raw\2026-07-26_065030_Final-Daily-AI-News-Digest.md
content_hash: e1d3c9f68af402bc53bd974ef7d94c478f9b10eb48d2b4438f0a05ef9a863409
normalized_title_hash: a2e2a89e35d1748c
canonical_url_hash: 93f6017ee09307cf
tags:
- New
- Research
entities: []
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-25-28-9m-parameter-llm-runs-locally-on-an-8-esp32-microcontroll
- 2026-06-01-minimax-releases-m3-an-open-weight-model-targeting-frontier
- 2026-08-03-2-deepseek-v4-flash-rated-the-cheapest-well-known-model-to-r
- 2026-09-17-prismml-raises-22-25m-seed-for-a-deliberately-small-language
- 2026-08-20-meet-s1-mini-superwhisper-s-462-mb-open-weights-text-normali
embedding_id: 2026-07-25-28-9m-parameter-llm-now-runs-locally-on-an-8-esp32-microcont
event_name: ''
---

# 28.9M-Parameter LLM Now Runs Locally on an $8 ESP32 Microcontroller

A developer ran a 28.9M-parameter TinyStories model entirely on an ESP32-S3 (~$8, 512KB SRAM), generating text at ~9.5 tokens/sec with nothing sent to a server. Uses per-layer embeddings in flash to pack more capacity onto the device. A pointed reminder that useful edge inference is migrating onto commodity, sub-$10 hardware.

<!-- graph:start -->
## Connections

**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-07-25-28-9m-parameter-llm-runs-locally-on-an-8-esp32-microcontroll]] · [[2026-06-01-minimax-releases-m3-an-open-weight-model-targeting-frontier]] · [[2026-08-03-2-deepseek-v4-flash-rated-the-cheapest-well-known-model-to-r]] · [[2026-09-17-prismml-raises-22-25m-seed-for-a-deliberately-small-language]] · [[2026-08-20-meet-s1-mini-superwhisper-s-462-mb-open-weights-text-normali]]
<!-- graph:end -->
