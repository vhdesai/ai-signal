---
article_id: 2026-07-20-google-works-on-frozen-v2-chip-to-improve-gemini-inference-e
title: Google works on “Frozen v2” chip to improve Gemini inference efficiency
date: '2026-07-20'
source: TechCrunch
url_original: null
url_canonical: https://techcrunch.com/2026/07/20/google-is-working-on-a-new-ai-chip-designed-to-make-gemini-more-efficient/
url_status: repaired
digest_source: digests\raw\2026-07-21-daily-ai-news-digest.md
content_hash: dc33f09ef2366833788989042176e4be968601e79018b091cb6178fec97ce10c
normalized_title_hash: 2dd3abeb3acbe91b
canonical_url_hash: ''
tags:
- Ai chips
- Google
- Efficiency
entities:
- Google
- NVIDIA
themes:
- datacenter-infrastructure
- infrastructure-investments
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-21-google-reportedly-building-frozen-v2-a-gemini-specific-chip
- 2026-07-20-google-plans-new-frozen-chip-to-run-ai-models-more-efficient
- 2026-05-08-alphaevolve-coming-to-google-cloud-enterprise-gemini-powered
- 2026-05-19-google-releases-gemini-3-5-flash-agent-optimized-efficiency
- 2026-05-03-google-s-unreleased-gemini-3-2-flash-surfaces-on-eleuther-ai
embedding_id: 2026-07-20-google-works-on-frozen-v2-chip-to-improve-gemini-inference-e
event_name: ''
---

# Google works on “Frozen v2” chip to improve Gemini inference efficiency

TechCrunch reports that Alphabet is designing a new server chip, internally called Frozen v2, to make Gemini models run more efficiently. The Information reported that the chip could be six to 10 times more efficient than current Google AI chips on tokens generated per unit of power. The broader significance is that inference efficiency is becoming a strategic battleground as AI spend faces investor scrutiny and hyperscalers try to reduce reliance on NVIDIA. URL: https://techcrunch.com/2026/07/20/google-is-working-on-a-new-ai-chip-designed-to-make-gemini-more-efficient/

<!-- graph:start -->
## Connections

**Entities:** [[Google]] · [[NVIDIA]]
**Topics:** [[Infrastructure & Compute]] · [[Infrastructure Investments]]
**Related:** [[2026-07-21-google-reportedly-building-frozen-v2-a-gemini-specific-chip]] · [[2026-07-20-google-plans-new-frozen-chip-to-run-ai-models-more-efficient]] · [[2026-05-08-alphaevolve-coming-to-google-cloud-enterprise-gemini-powered]] · [[2026-05-19-google-releases-gemini-3-5-flash-agent-optimized-efficiency]] · [[2026-05-03-google-s-unreleased-gemini-3-2-flash-surfaces-on-eleuther-ai]]
<!-- graph:end -->
