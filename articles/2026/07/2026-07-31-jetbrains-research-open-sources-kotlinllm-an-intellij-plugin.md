---
article_id: 2026-07-31-jetbrains-research-open-sources-kotlinllm-an-intellij-plugin
title: JetBrains Research open-sources KotlinLLM, an IntelliJ plugin with runtime
  “Smart macros”
date: '2026-07-31'
source: MarkTechPost
url_original: https://www.marktechpost.com/2026/07/31/jetbrains-research-open-sources-kotlinllm-intellij-plugin-kotlin-runtime-llm/
url_canonical: https://www.marktechpost.com/2026/07/31/jetbrains-research-open-sources-kotlinllm-intellij-plugin-kotlin-runtime-llm/
url_status: found
digest_source: digests\raw\2026-08-01_061413_Inbox_Daily AI News Digest - August 1,
  2026.md
content_hash: 5ad8bb41d769528ac67ce0dcb381809c9909155b466045f29d6af2cb242e9467
normalized_title_hash: 23b752cd0c270f05
canonical_url_hash: 6cbdffa80a320f5d
tags:
- New
entities: []
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-14-poetiq-meta-system-improves-every-llm-tested-on-livecodebenc
- 2026-08-06-microsoft-open-sources-unit-test-ai-agent-claims-gains-over
- 2026-06-24-openai-and-broadcom-unveil-jalape-o-openai-s-first-custom-in
- 2026-08-15-fine-tuning-tool-calling-llms-a-complete-guide-using-xyz-aqu
- 2026-05-26-andrej-karpathy-skills-github-repo-packages-karpathy-s-llm-t
embedding_id: 2026-07-31-jetbrains-research-open-sources-kotlinllm-an-intellij-plugin
event_name: ''
---

# JetBrains Research open-sources KotlinLLM, an IntelliJ plugin with runtime “Smart macros”

JetBrains Research open-sourced KotlinLLM (Apache-2.0), an IntelliJ IDEA plugin that adds “Smart macros” whose bodies are LLM-generated Kotlin source rather than live model calls. The plugin captures runtime values via the JVM Debug Interface, asks an agent for a narrow code update, compiles it and hot-reloads the class so covered paths run as plain Kotlin. On an adapted Spring Petclinic project it reported 24/24 scenarios with 100% hot-reload success and about 1% runtime overhead.

<!-- graph:start -->
## Connections

**Topics:** [[Corporate Moves]]
**Related:** [[2026-05-14-poetiq-meta-system-improves-every-llm-tested-on-livecodebenc]] · [[2026-08-06-microsoft-open-sources-unit-test-ai-agent-claims-gains-over]] · [[2026-06-24-openai-and-broadcom-unveil-jalape-o-openai-s-first-custom-in]] · [[2026-08-15-fine-tuning-tool-calling-llms-a-complete-guide-using-xyz-aqu]] · [[2026-05-26-andrej-karpathy-skills-github-repo-packages-karpathy-s-llm-t]]
<!-- graph:end -->
