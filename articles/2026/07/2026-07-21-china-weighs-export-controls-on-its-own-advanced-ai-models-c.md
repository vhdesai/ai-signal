---
article_id: 2026-07-21-china-weighs-export-controls-on-its-own-advanced-ai-models-c
title: China weighs export controls on its own advanced AI models, chips, and data
date: '2026-07-21'
source: The Next Web
url_original: https://thenextweb.com/news/china-ai-model-chip-export-controls-ft-report
url_canonical: https://thenextweb.com/news/china-ai-model-chip-export-controls-ft-report
url_status: found
digest_source: digests\raw\2026-07-22_065707_Final-Daily-AI-News-Digest.md
content_hash: e05f285d5cddfa032562c20b62640b63f237cec45dbd00015e2de8bcaf9265ea
normalized_title_hash: eeac936e39dc917f
canonical_url_hash: 910b85292142e032
tags:
- Trending
- Export controls
entities:
- Alibaba
- ByteDance
themes:
- policy-regulation
cross_cutting_topics:
- china-compete
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-07-beijing-reportedly-weighs-restricting-overseas-access-to-adv
- 2026-07-06-beijing-weighs-restricting-overseas-access-to-advanced-chine
embedding_id: 2026-07-21-china-weighs-export-controls-on-its-own-advanced-ai-models-c
event_name: ''
---

# China weighs export controls on its own advanced AI models, chips, and data

China is reportedly consulting Alibaba, ByteDance, Zhipu, and others about restricting foreign access to advanced models, semiconductor technology, and training data. The move mirrors U.S. tactics by treating Chinese AI IP as a strategic national asset. It could affect developers and enterprises building on Chinese open-weight models, especially if Beijing narrows cross-border access.

<!-- graph:start -->
## Connections

**Entities:** [[Alibaba]] · [[ByteDance]]
**Topics:** [[Policy & Regulation]] · [[Global AI Race]]
**Related:** [[2026-07-07-beijing-reportedly-weighs-restricting-overseas-access-to-adv]] · [[2026-07-06-beijing-weighs-restricting-overseas-access-to-advanced-chine]]
<!-- graph:end -->
