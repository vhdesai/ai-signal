---
article_id: 2026-09-08-quanta-the-proof-is-lean-verified-but-scope-and-precedence-a
title: 'Quanta: the proof is Lean-verified, but scope and precedence are contested'
date: '2026-09-08'
source: Quanta Magazine
url_original: https://www.quantamagazine.org/ai-has-solved-one-of-maths-1-million-millennium-prize-problems-20260908/
url_canonical: https://www.quantamagazine.org/ai-has-solved-one-of-maths-1-million-millennium-prize-problems-20260908/
url_status: found
digest_source: digests\raw\2026-09-09_073521_Final-Daily-AI-News-Digest.md
content_hash: ed3e297c757df7c7a4aa9b3d777aea2860b26135bedca1da53f5b21eb7cbab65
normalized_title_hash: f18b2f3a687ded25
canonical_url_hash: 23f0f9c47f987f1c
tags:
- Research
- Evaluation
entities:
- Anthropic
- OpenAI
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-09-09-openai-s-agent-generated-navier-stokes-proof-draws-lean-veri
- 2026-09-08-openai-publishes-agent-generated-proof-of-finite-time-singul
- 2026-09-09-credit-dispute-over-the-navier-stokes-result-exposes-an-acad
- 2026-09-09-openai-claims-10-000-agent-system-produced-navier-stokes-pro
- 2026-09-08-credit-dispute-over-the-navier-stokes-result-exposes-an-acad
embedding_id: 2026-09-08-quanta-the-proof-is-lean-verified-but-scope-and-precedence-a
event_name: ''
---

# Quanta: the proof is Lean-verified, but scope and precedence are contested

Quanta reports the OpenAI Navier–Stokes result has been formally checked in Lean, giving mathematicians confidence in its correctness, and frames it as by a significant margin the most important AI-derived proof to date. It also notes OpenAI's announcement came 12 hours after NYU's Tristan Buckmaster and Anthropic's Levent Alpöge announced resolution of several closely related problems, and that both efforts leaned heavily on a strategy developed by Diego Córdoba and Luis Martínez-Zoroa. Princeton's Charles Fefferman, who authored the Clay problem description, credited Córdoba and Martínez-Zoroa as the central figures. Analysts elsewhere note OpenAI's result addresses the forced equations, not the unforced formulation the Clay criteria specify.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[OpenAI]]
**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-09-09-openai-s-agent-generated-navier-stokes-proof-draws-lean-veri]] · [[2026-09-08-openai-publishes-agent-generated-proof-of-finite-time-singul]] · [[2026-09-09-credit-dispute-over-the-navier-stokes-result-exposes-an-acad]] · [[2026-09-09-openai-claims-10-000-agent-system-produced-navier-stokes-pro]] · [[2026-09-08-credit-dispute-over-the-navier-stokes-result-exposes-an-acad]]
<!-- graph:end -->
