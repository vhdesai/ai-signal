---
article_id: 2026-09-02-trust3-ai-extends-agent-security-controls-to-the-databricks
title: Trust3 AI extends agent security controls to the Databricks Unity AI Gateway
date: '2026-09-02'
source: Business Wire via Yahoo Finance
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-09-03_060607_Inbox_Daily AI News Digest – September
  3, 2026.md
content_hash: 8b85062bc4594d63824c2a665784e796217528c21aef887ce94717f4146ee32c
normalized_title_hash: 6fff17614eaa460a
canonical_url_hash: ''
tags: []
entities:
- Databricks
- Snowflake
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-04-23-databricks-and-palantir-each-announce-new-enterprise-agent-f
- 2026-09-06-databricks-launches-context-engineer-certification-and-expan
- 2026-06-15-databricks-closes-dais-2026-with-genie-one-agent-bricks-and
- event-microsoft-build-2026-microsoft-build-2026-security-trust-governance-and-responsib
- 2026-06-22-google-is-reportedly-building-a-trusted-directory-for-ai-age
embedding_id: 2026-09-02-trust3-ai-extends-agent-security-controls-to-the-databricks
event_name: ''
---

# Trust3 AI extends agent security controls to the Databricks Unity AI Gateway

Trust3 AI, founded by the creators of Apache Ranger, extended purpose-based agent security controls to the Databricks Unity AI Gateway, enforcing policy over both what an agent may do and which data it may reach. Coverage spans agents built in Databricks Agent Bricks as well as external agents, with the same controls available across Azure AI Gateway, Snowflake Cortex and Bedrock AgentCore. Note: sourced from a paid press release.

<!-- graph:start -->
## Connections

**Entities:** [[Databricks]] · [[Snowflake]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-04-23-databricks-and-palantir-each-announce-new-enterprise-agent-f]] · [[2026-09-06-databricks-launches-context-engineer-certification-and-expan]] · [[2026-06-15-databricks-closes-dais-2026-with-genie-one-agent-bricks-and]] · [[event-microsoft-build-2026-microsoft-build-2026-security-trust-governance-and-responsib]] · [[2026-06-22-google-is-reportedly-building-a-trusted-directory-for-ai-age]]
<!-- graph:end -->
