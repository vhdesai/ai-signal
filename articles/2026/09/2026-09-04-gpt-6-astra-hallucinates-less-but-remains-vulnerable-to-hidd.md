---
article_id: 2026-09-04-gpt-6-astra-hallucinates-less-but-remains-vulnerable-to-hidd
title: GPT-6 Astra hallucinates less but remains vulnerable to hidden prompt injection
date: '2026-09-04'
source: THE DECODER
url_original: https://the-decoder.com/openais-gpt-6-astra-hallucinates-less-but-remains-vulnerable-to-hidden-prompt-injections/
url_canonical: https://the-decoder.com/openais-gpt-6-astra-hallucinates-less-but-remains-vulnerable-to-hidden-prompt-injections/
url_status: ok
digest_source: digests\raw\2026-09-05_061544_Inbox_Daily AI News Digest - September
  5, 2026.md
content_hash: c7698348a89cf2ad2e5f3ee5be8816dcc0d11c1748257281ac4ae800d82e6110
normalized_title_hash: 66ee1b4f71e63e26
canonical_url_hash: 14e863daf7d51d4e
tags:
- Hot
entities:
- OpenAI
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-09-03-hot-breaking-openai-launches-gpt-6-astra-a-computer-use-flag
- 2026-09-05-openai-publishes-gpt-6-astra-prompting-guide-including-a-slo
- 2026-09-05-gpt-6-astra-draws-scrutiny-over-cybersecurity-capability-sco
- 2026-09-04-gpt-6-astra-a-new-generation-of-intelligence
embedding_id: 2026-09-04-gpt-6-astra-hallucinates-less-but-remains-vulnerable-to-hidd
event_name: ''
---

# GPT-6 Astra hallucinates less but remains vulnerable to hidden prompt injection

OpenAI's GPT-6 Astra hallucinates less than its predecessor and blocks 99.99% of direct prompt injections. But when malicious instructions are hidden inside documents the model reads, it is still compromised in roughly 8.5% of scenarios, versus 4.8% for Claude Opus 5. The analysis argues indirect-injection success rates remain unacceptably high for autonomous agents operating on real enterprise data.

<!-- graph:start -->
## Connections

**Entities:** [[OpenAI]]
**Topics:** [[Policy & Regulation]]
**Related:** [[2026-09-03-hot-breaking-openai-launches-gpt-6-astra-a-computer-use-flag]] · [[2026-09-05-openai-publishes-gpt-6-astra-prompting-guide-including-a-slo]] · [[2026-09-05-gpt-6-astra-draws-scrutiny-over-cybersecurity-capability-sco]] · [[2026-09-04-gpt-6-astra-a-new-generation-of-intelligence]]
<!-- graph:end -->
