---
article_id: 2026-09-21-nvidia-s-sol-pi-ai-discovered-harness-optimizations-cut-codi
title: 'NVIDIA''s SoL-Pi: AI-Discovered Harness Optimizations Cut Coding-Agent Token
  Traffic Up to 49%'
date: '2026-09-21'
source: MarkTechPost]
url_original: https://www.marktechpost.com/2026/09/21/nvidia-sol-pi-ai-discovered-harness-optimizations-coding-agents/
url_canonical: https://www.marktechpost.com/2026/09/21/nvidia-sol-pi-ai-discovered-harness-optimizations-coding-agents/
url_status: ok
digest_source: digests\raw\2026-09-22_090723_Inbox_Fw Daily AI News Digest - September
  22, 2026.md
content_hash: 6b7657c23bb48956596b4fb3c92a49eb1a56aa331980c56b7502311d533e4569
normalized_title_hash: 1dc8119f25de8126
canonical_url_hash: 50f2512d301d493f
tags: []
entities:
- NVIDIA
themes:
- datacenter-infrastructure
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-11-nvidia-switchyard-mid-task-model-router-cuts-agent-costs-to
- 2026-08-21-nvidia-research-the-agent-harness-not-the-base-model-drives
- 2026-09-12-context-engineering-inside-the-agent-harness-four-mechanisms
embedding_id: 2026-09-21-nvidia-s-sol-pi-ai-discovered-harness-optimizations-cut-codi
event_name: ''
---

# NVIDIA's SoL-Pi: AI-Discovered Harness Optimizations Cut Coding-Agent Token Traffic Up to 49%

Researchers from NVIDIA, NTU, and MIT released SoL-Pi, four harness-level efficiency mechanisms — Action Fusion, Online Context Compact, ObservationPack, and an Evidence-Preserving Reducer — for the open-source Pi coding agent. Notably, the mechanisms were discovered by an AI running automated research loops across 152 proposed directions and 535 executable environments, totaling more than 3,000 runs and 60,000 agent-environment interactions. On the held-out 51-task EdgeBench, SoL-Pi cuts token traffic 44.7–49.0% and API cost roughly 33% while retaining about 94% of Pi's score on both GPT-5.6 Sol and Opus 5. It ships on GitHub under NVlabs as an MIT-licensed extension to unmodified Pi — agent efficiency is moving into the harness layer, and it can be captured without changing models. Academic Research HOT ACADEMIC

<!-- graph:start -->
## Connections

**Entities:** [[NVIDIA]]
**Topics:** [[Infrastructure & Compute]]
**Related:** [[2026-08-11-nvidia-switchyard-mid-task-model-router-cuts-agent-costs-to]] · [[2026-08-21-nvidia-research-the-agent-harness-not-the-base-model-drives]] · [[2026-09-12-context-engineering-inside-the-agent-harness-four-mechanisms]]
<!-- graph:end -->
