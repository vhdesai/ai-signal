---
article_id: 2026-09-18-mit-researchers-map-extreme-weather-risks-and-build-ai-tools
title: MIT Researchers Map Extreme Weather Risks — and Build AI Tools to Act on Them
date: '2026-09-18'
source: MIT News
url_original: https://news.mit.edu/2026/mit-researchers-mapping-extreme-weather-risks-building-tools-to-act-0918
url_canonical: https://news.mit.edu/2026/mit-researchers-mapping-extreme-weather-risks-building-tools-to-act-0918
url_status: broken
digest_source: digests\raw\2026-09-19_060243_Inbox_Daily AI News Digest - September
  19, 2026.md
content_hash: 65702d5e878cef3a7627696fb81b4db2672d3b75be768e3b1e5f3f4cc4d6ced8
normalized_title_hash: d373c354f0c30d6b
canonical_url_hash: a93c5d6fccb675f9
tags: []
entities: []
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-24-mit-generating-scenarios-for-extreme-events-without-extreme
- 2026-08-24-mit-builds-a-tool-that-generates-unprecedented-extreme-event
- 2026-09-24-mit-researchers-assess-the-promise-and-peril-of-visual-ai-fo
- 2026-05-13-an-open-access-review-article-published-today-in-discover-ar
- 2026-09-24-mit-s-senseable-city-lab-weighs-the-promise-and-peril-of-vis
embedding_id: 2026-09-18-mit-researchers-map-extreme-weather-risks-and-build-ai-tools
event_name: ''
---

# MIT Researchers Map Extreme Weather Risks — and Build AI Tools to Act on Them

MIT published a four-year progress report on its Climate Grand Challenges project “Preparing for a New World of Weather and Climate Extremes,” covering 40+ faculty and student researchers, 29 published papers, and datasets and digital tools now deployed or near deployment. Work spans hurricane and convective-storm risk estimation (Kerry Emanuel) and humidity-driven extreme-rainfall modeling (Paul O'Gorman). The AI-relevant thread is Miho Mazereeuw's Urban Risk Lab, whose planning tools let city staff query flood models in natural language — for example, which schools stay driest across scenarios. A separate workstream optimizes siting of wind, solar and storage against weather extremes.

<!-- graph:start -->
## Connections

**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-08-24-mit-generating-scenarios-for-extreme-events-without-extreme]] · [[2026-08-24-mit-builds-a-tool-that-generates-unprecedented-extreme-event]] · [[2026-09-24-mit-researchers-assess-the-promise-and-peril-of-visual-ai-fo]] · [[2026-05-13-an-open-access-review-article-published-today-in-discover-ar]] · [[2026-09-24-mit-s-senseable-city-lab-weighs-the-promise-and-peril-of-vis]]
<!-- graph:end -->
