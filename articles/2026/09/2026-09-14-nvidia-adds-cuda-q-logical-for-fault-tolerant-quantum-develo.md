---
article_id: 2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-develo
title: NVIDIA Adds CUDA-Q Logical for Fault-Tolerant Quantum Development
date: '2026-09-14'
source: Business Insider
url_original: https://markets.businessinsider.com/news/stocks/nvidia-expands-open-source-cuda-q-platform-for-fault-tolerant-quantum-computing-1036542900
url_canonical: https://markets.businessinsider.com/news/stocks/nvidia-expands-open-source-cuda-q-platform-for-fault-tolerant-quantum-computing-1036542900
url_status: ok
digest_source: digests\raw\2026-09-14_062355_Inbox_Daily AI News Digest - September
  14, 2026.md
content_hash: 8b07cb359982e3649ea2567877ea80be02134e5c30f7b3701e6927d720310fe9
normalized_title_hash: 4002e7e0350d4b60
canonical_url_hash: e4f0bc7257ff14de
tags: []
entities:
- NVIDIA
themes:
- datacenter-infrastructure
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-codesi
related_article_ids: []
embedding_id: 2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-develo
event_name: ''
---

# NVIDIA Adds CUDA-Q Logical for Fault-Tolerant Quantum Development

NVIDIA expanded its open-source CUDA-Q platform with CUDA-Q Logical, an orchestration layer for designing and verifying applications on fault-tolerant quantum systems. Fermilab reported compressing fault-tolerant architecture development from five months to three weeks — a 7x speedup — and Iceberg Quantum modeled 1,000 logical qubits using 150,000 physical qubits, roughly 10x fewer than prior estimates for Diraq's qubits. Sandia National Laboratories' QUOPS benchmark, which measures hardware readiness for utility-scale applications, is now available in CUDA-Q. Early adopters include Fermilab, Infleqtion, IQM, Quantum Motion and Sandia. RESEARCH

<!-- graph:start -->
## Connections

**Entities:** [[NVIDIA]]
**Topics:** [[Infrastructure & Compute]]
**Canonical:** [[2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-codesi]]
<!-- graph:end -->
