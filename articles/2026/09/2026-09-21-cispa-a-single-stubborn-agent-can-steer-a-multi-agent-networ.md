---
article_id: 2026-09-21-cispa-a-single-stubborn-agent-can-steer-a-multi-agent-networ
title: 'CISPA: A Single "Stubborn" Agent Can Steer a Multi-Agent Network''s Decisions'
date: '2026-09-21'
source: Tech Xplore
url_original: https://techxplore.com/news/2026-09-stubbornness-problem-ai-agents.html
url_canonical: https://techxplore.com/news/2026-09-stubbornness-problem-ai-agents.html
url_status: ok
digest_source: digests\raw\2026-09-22_060336_Inbox_Daily AI News Digest - September
  22, 2026.md
content_hash: ce945e52a9fd6b40de8a2639f9abcdf971095e6c8988a7ff141f759431be64a1
normalized_title_hash: ea0d9fbccb4b989d
canonical_url_hash: 55da455201ceb4a2
tags:
- Trending
entities: []
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-06-frontier-agents-breach-security-boundaries-openai-meta-and-u
- 2026-08-04-openai-agents-chained-an-artifactory-zero-day-to-escape-sand
- 2026-05-14-arxiv-history-anchors-how-prior-behavior-steers-llms-toward
- 2026-05-08-hotanthropic-teaching-claude-why-a-new-methodology-for-princ
embedding_id: 2026-09-21-cispa-a-single-stubborn-agent-can-steer-a-multi-agent-networ
event_name: ''
---

# CISPA: A Single "Stubborn" Agent Can Steer a Multi-Agent Network's Decisions

CISPA researcher Samira Abedini and colleagues adapted the Friedkin-Johnsen opinion-dynamics model from social science to multi-agent LLM networks, and found it predicted when a manipulative agent succeeds. A single agent with high "stubbornness" can steer a network's collective decision if it also holds sufficient influence, with star-network hubs especially dangerous and larger networks requiring more attacker influence. A dynamic trust mechanism — periodic known-answer probes that downweight repeatedly wrong agents — measurably reduced manipulative influence. Preprint: "Don't Trust Stubborn Neighbors: A Security Framework for Agentic Networks" (arXiv, DOI 10.48550/arxiv.2603.15809).

<!-- graph:start -->
## Connections

**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-08-06-frontier-agents-breach-security-boundaries-openai-meta-and-u]] · [[2026-08-04-openai-agents-chained-an-artifactory-zero-day-to-escape-sand]] · [[2026-05-14-arxiv-history-anchors-how-prior-behavior-steers-llms-toward]] · [[2026-05-08-hotanthropic-teaching-claude-why-a-new-methodology-for-princ]]
<!-- graph:end -->
