---
article_id: 2026-09-14-infleqtion-reports-5x-better-code-rate-in-quantum-error-corr
title: Infleqtion Reports 5x Better Code Rate in Quantum Error Correction
date: '2026-09-14'
source: Infleqtion
url_original: https://infleqtion.com/infleqtion-advances-fault-tolerant-quantum-computing-software-with-nvidia-cuda-q-logical/
url_canonical: https://infleqtion.com/infleqtion-advances-fault-tolerant-quantum-computing-software-with-nvidia-cuda-q-logical/
url_status: broken
digest_source: digests\raw\2026-09-14_062355_Inbox_Daily AI News Digest - September
  14, 2026.md
content_hash: 7aea8f2e354baf1284f096f02e2052dba1c66069902d241c7e5d6a4d77e84eb5
normalized_title_hash: 73453bba4991a86e
canonical_url_hash: 12f3ede31a9350bd
tags: []
entities: []
themes:
- datacenter-infrastructure
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-09-14-infleqtion-reports-5x-reduction-in-physical-qubits-per-logic
- 2026-09-14-nvidia-expands-open-source-cuda-q-with-cuda-q-logical-for-fa
- 2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-develo
- 2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-codesi
- 2026-09-14-nvidia-expands-cuda-q-to-fault-tolerant-quantum-processors
embedding_id: 2026-09-14-infleqtion-reports-5x-better-code-rate-in-quantum-error-corr
event_name: ''
---

# Infleqtion Reports 5x Better Code Rate in Quantum Error Correction

Using an early-access build of CUDA-Q Logical with its open-source qLDPC library, Infleqtion constructed and validated a high-rate error-correction code requiring roughly six physical data qubits per logical qubit — about a fivefold improvement in code rate over the surface-code baseline used for comparison. The sub-10:1 ratio is the company's stated architectural target for near-term neutral-atom systems. Physical-qubit overhead remains the central cost obstacle between current hardware and commercially useful quantum computing. Infleqtion says portions of the integration were developed with AI assistance.

<!-- graph:start -->
## Connections

**Topics:** [[Infrastructure & Compute]]
**Related:** [[2026-09-14-infleqtion-reports-5x-reduction-in-physical-qubits-per-logic]] · [[2026-09-14-nvidia-expands-open-source-cuda-q-with-cuda-q-logical-for-fa]] · [[2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-develo]] · [[2026-09-14-nvidia-adds-cuda-q-logical-for-fault-tolerant-quantum-codesi]] · [[2026-09-14-nvidia-expands-cuda-q-to-fault-tolerant-quantum-processors]]
<!-- graph:end -->
