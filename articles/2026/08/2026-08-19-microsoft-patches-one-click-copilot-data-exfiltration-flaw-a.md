---
article_id: 2026-08-19-microsoft-patches-one-click-copilot-data-exfiltration-flaw-a
title: Microsoft Patches One-Click Copilot Data-Exfiltration Flaw After Eight Months
date: '2026-08-19'
source: Computerworld
url_original: https://www.computerworld.com/article/4211325/microsoft-finally-patches-critical-one-click-copilot-vulnerability-more-than-eight-months-after-learning-of-it.html
url_canonical: https://www.computerworld.com/article/4211325/microsoft-finally-patches-critical-one-click-copilot-vulnerability-more-than-eight-months-after-learning-of-it.html
url_status: found
digest_source: digests\raw\2026-08-20_065002_Final-Daily-AI-News-Digest.md
content_hash: ca0c4f89ac9eee406d85399f97b87cd648c1c1ceedb8fab029896473e66e930d
normalized_title_hash: 9a465494673f9b07
canonical_url_hash: 1fef709088455404
tags:
- Breaking
- Security
entities:
- Microsoft
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-20-varonis-details-cosnitch-one-click-vulnerability-in-microsof
- 2026-07-30-new-microsoft-copilot-flaws-reportedly-could-leak-customer-s
- 2026-07-15-microsoft-patches-record-570-vulnerabilities-citing-ai-assis
- 2026-08-17-github-goes-down-worldwide-degrading-actions-copilot-issues
embedding_id: 2026-08-19-microsoft-patches-one-click-copilot-data-exfiltration-flaw-a
event_name: ''
---

# Microsoft Patches One-Click Copilot Data-Exfiltration Flaw After Eight Months

Microsoft shipped a fix for "CoSnitch," a vulnerability chain that let a single click exfiltrate enterprise email and calendar data through Copilot's connected accounts, with no confirmation prompt and the option to plant persistent instructions. The flaw was reported nearly eight months before the patch landed. The governance takeaway: AI assistants with connector access need the same patch SLAs, monitoring and exposure tracking as core productivity infrastructure.

<!-- graph:start -->
## Connections

**Entities:** [[Microsoft]]
**Topics:** [[Policy & Regulation]]
**Related:** [[2026-08-20-varonis-details-cosnitch-one-click-vulnerability-in-microsof]] · [[2026-07-30-new-microsoft-copilot-flaws-reportedly-could-leak-customer-s]] · [[2026-07-15-microsoft-patches-record-570-vulnerabilities-citing-ai-assis]] · [[2026-08-17-github-goes-down-worldwide-degrading-actions-copilot-issues]]
<!-- graph:end -->
