---
article_id: 2026-08-07-anthropic-makes-auto-mode-the-default-in-claude-code-across
title: Anthropic makes "auto mode" the default in Claude Code across Pro, Max, and
  Team
date: '2026-08-07'
source: Anthropic / 9to5Mac
url_original: https://claude.com/blog/auto-mode-default-in-claude-code
url_canonical: https://claude.com/blog/auto-mode-default-in-claude-code
url_status: found
digest_source: digests\raw\2026-08-08_060927_Inbox_Daily AI News Digest - August 8,
  2026.md
content_hash: 62491b1cac46339886e20ce3c97de486f99bf4631454846fd7b88833d9014e4b
normalized_title_hash: 4fecfb915c05430c
canonical_url_hash: bf86c80d4ed8f877
tags:
- Trending
entities:
- Amazon
- Anthropic
- Google
- Microsoft
themes:
- company-storylines
- infrastructure-investments
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-07-auto-mode-is-now-the-default-in-claude-code-for-pro-max-and
- 2026-08-09-anthropic-makes-claude-code-auto-mode-the-default
- 2026-08-09-anthropic-makes-claude-code-auto-mode-default-catches-89-of
- 2026-08-09-anthropic-makes-claude-code-auto-mode-the-default-for-pro-ma
- 2026-08-09-anthropic-makes-claude-code-s-auto-mode-the-default-for-pro
embedding_id: 2026-08-07-anthropic-makes-auto-mode-the-default-in-claude-code-across
event_name: ''
---

# Anthropic makes "auto mode" the default in Claude Code across Pro, Max, and Team

Starting August 14, new Claude Code sessions on Pro, Max, and Team plans will run in auto mode, replacing repeated approval prompts with a classifier that vets each tool call for irreversible, destructive, or out-of-bounds actions. Anthropic said it stopped charging for the classifier’s token overhead effective August 7. It cited a 1,053-person study in which human reviewers caught only 13.6% of dangerous commands versus 89% for auto mode, while cautioning that classifiers "cannot eliminate risk." Auto mode remains opt-in on Enterprise, the API, and cloud partners including AWS Bedrock, Google Cloud, and Microsoft Foundry.

<!-- graph:start -->
## Connections

**Entities:** [[Amazon]] · [[Anthropic]] · [[Google]] · [[Microsoft]]
**Topics:** [[Corporate Moves]] · [[Infrastructure Investments]]
**Related:** [[2026-08-07-auto-mode-is-now-the-default-in-claude-code-for-pro-max-and]] · [[2026-08-09-anthropic-makes-claude-code-auto-mode-the-default]] · [[2026-08-09-anthropic-makes-claude-code-auto-mode-default-catches-89-of]] · [[2026-08-09-anthropic-makes-claude-code-auto-mode-the-default-for-pro-ma]] · [[2026-08-09-anthropic-makes-claude-code-s-auto-mode-the-default-for-pro]]
<!-- graph:end -->
