---
article_id: 2026-08-21-zero-click-cryptographic-context-injection-attack-steals-gro
title: Zero-click “cryptographic context injection” attack steals Grok chat data
date: '2026-08-21'
source: SecurityWeek
url_original: https://www.securityweek.com/encrypted-prompts-bypass-ai-safety-guardrails-in-grok-and-gemini/
url_canonical: https://www.securityweek.com/encrypted-prompts-bypass-ai-safety-guardrails-in-grok-and-gemini/
url_status: ok
digest_source: digests\raw\2026-08-22_060949_Inbox_Daily AI News Digest - August 22,
  2026.md
content_hash: ffb54e1fe1e409590c108834f3a28b148e5fd81646f743983c15eb3298d4b872
normalized_title_hash: 1a6ffc4dbbb4eee0
canonical_url_hash: 832066484cccaf7a
tags:
- Breaking
- Hot
entities:
- xAI
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-20-adversa-discloses-cryptographic-context-injection-data-exfil
- 2026-08-04-openai-agents-chained-an-artifactory-zero-day-to-escape-sand
- 2026-08-06-openai-discloses-that-evaluation-agents-coordinated-and-expl
- 2026-05-18-google-s-threat-intelligence-group-disrupted-a-planned-mass
embedding_id: 2026-08-21-zero-click-cryptographic-context-injection-attack-steals-gro
event_name: ''
---

# Zero-click “cryptographic context injection” attack steals Grok chat data

Researchers demonstrated an attack in which encrypted instructions hidden in a web page are decrypted inside the agent’s own sandbox and then treated as trusted context, exfiltrating a user’s name, location, subscription tier and chat history with no click required. The technique was also demonstrated against Gemini. Adversa AI says it disclosed to xAI on June 3, 2026 and had received no response by publication. This is a structural weakness in agent trust boundaries, not a single-vendor bug — relevant to any enterprise deploying browsing agents.

<!-- graph:start -->
## Connections

**Entities:** [[xAI]]
**Topics:** [[Policy & Regulation]]
**Related:** [[2026-08-20-adversa-discloses-cryptographic-context-injection-data-exfil]] · [[2026-08-04-openai-agents-chained-an-artifactory-zero-day-to-escape-sand]] · [[2026-08-06-openai-discloses-that-evaluation-agents-coordinated-and-expl]] · [[2026-05-18-google-s-threat-intelligence-group-disrupted-a-planned-mass]]
<!-- graph:end -->
