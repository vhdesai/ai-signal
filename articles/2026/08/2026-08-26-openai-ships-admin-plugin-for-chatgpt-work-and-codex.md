---
article_id: 2026-08-26-openai-ships-admin-plugin-for-chatgpt-work-and-codex
title: OpenAI Ships Admin Plugin for ChatGPT Work and Codex
date: '2026-08-26'
source: Techgenyz
url_original: https://techgenyz.com/openai-admin-plugin-workspace-chatgpt-work-codex/
url_canonical: https://techgenyz.com/openai-admin-plugin-workspace-chatgpt-work-codex/
url_status: ok
digest_source: digests\raw\2026-08-26_061028_Inbox_Daily AI News Digest - August 26,
  2026.md
content_hash: 63743142c1c3b0cb65dab53fae619d600d51aa984da7851660ef9d845645fbed
normalized_title_hash: be810999b30a77dc
canonical_url_hash: a3ed3ccf1590f11d
tags:
- New
entities:
- OpenAI
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-10-openai-introduces-chatgpt-work-a-gpt-5-6-powered-super-app
embedding_id: 2026-08-26-openai-ships-admin-plugin-for-chatgpt-work-and-codex
event_name: ''
---

# OpenAI Ships Admin Plugin for ChatGPT Work and Codex

OpenAI launched an Admin plugin for ChatGPT Work and Codex, giving workspace administrators centralized control over users, permissions, usage limits, spending and automated workflows. The feature targets the governance gap that has slowed enterprise rollouts. Single-source at time of writing; treat details as developing.

<!-- graph:start -->
## Connections

**Entities:** [[OpenAI]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-07-10-openai-introduces-chatgpt-work-a-gpt-5-6-powered-super-app]]
<!-- graph:end -->
