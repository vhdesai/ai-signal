---
article_id: 2026-08-22-openai-cuts-gpt-5-6-sol-api-pricing-by-more-than-20
title: OpenAI cuts GPT-5.6 Sol API pricing by more than 20%
date: '2026-08-22'
source: explainX
url_original: https://www.explainx.ai/blog/openai-gpt-5-6-sol-api-price-cut-20-percent-august-2026
url_canonical: https://www.explainx.ai/blog/openai-gpt-5-6-sol-api-price-cut-20-percent-august-2026
url_status: found
digest_source: digests\raw\2026-08-22_060949_Inbox_Daily AI News Digest - August 22,
  2026.md
content_hash: 3f3f648a35ea0d2b43381d08c362746fcab7606e520c57c10c540e09afb580bf
normalized_title_hash: 8b26d9d3357fa978
canonical_url_hash: cf08d9bc71b28c26
tags:
- Trending
entities:
- Anthropic
- OpenAI
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-31-openai-cuts-gpt-5-6-prices-as-enterprise-token-costs-bite
- 2026-07-30-openai-cuts-gpt-5-6-api-pricing-up-to-80-and-adds-sol-fast
- 2026-08-21-openai-cuts-gpt-5-6-sol-api-and-credit-pricing-by-more-than
embedding_id: 2026-08-22-openai-cuts-gpt-5-6-sol-api-pricing-by-more-than-20
event_name: ''
---

# OpenAI cuts GPT-5.6 Sol API pricing by more than 20%

OpenAI is reducing API and credit pricing for GPT-5.6 Sol by over 20% for the next three months, to $4 per million input tokens and $20 per million output tokens. The cut applies to the API and to ChatGPT Work and Codex credits, but not to Pro, Plus or Business subscriptions. Commentary reads it as a direct competitive response to Anthropic’s enterprise pricing. Single-source via a secondary outlet quoting OpenAI’s own post — worth confirming against the official pricing page before planning around it.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[OpenAI]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-07-31-openai-cuts-gpt-5-6-prices-as-enterprise-token-costs-bite]] · [[2026-07-30-openai-cuts-gpt-5-6-api-pricing-up-to-80-and-adds-sol-fast]] · [[2026-08-21-openai-cuts-gpt-5-6-sol-api-and-credit-pricing-by-more-than]]
<!-- graph:end -->
