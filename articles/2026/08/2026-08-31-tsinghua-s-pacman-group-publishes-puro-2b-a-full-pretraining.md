---
article_id: 2026-08-31-tsinghua-s-pacman-group-publishes-puro-2b-a-full-pretraining
title: Tsinghua's PACMAN Group Publishes Puro-2B, a Full Pretraining Recipe for Consumer
  GPUs
date: '2026-08-31'
source: arXiv
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-09-01_081735_Inbox_Daily AI News Digest - September
  1, 2026.md
content_hash: 48d748c306bc3409b92d2611602909ff9fc664810cca1e2d9428a613c2cd7752
normalized_title_hash: fdc25c3c051992e6
canonical_url_hash: ''
tags: []
entities: []
themes:
- datacenter-infrastructure
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-31-tsinghua-s-pacman-group-releases-puro-2b-a-fully-open-recipe
- 2026-09-01-tsinghua-s-pacman-group-publishes-puro-2b-a-full-pretraining
- 2026-05-07-new-zaya1-8b-competitive-open-reasoning-model-trained-entire
- 2026-09-04-september-4-2026-the-information
- 2026-08-12-nvidia-details-serving-alibaba-s-2-4t-parameter-qwen3-8-mode
embedding_id: 2026-08-31-tsinghua-s-pacman-group-publishes-puro-2b-a-full-pretraining
event_name: ''
---

# Tsinghua's PACMAN Group Publishes Puro-2B, a Full Pretraining Recipe for Consumer GPUs

Tsinghua University's PACMAN group published Puro-2B, an open end-to-end recipe for pretraining a 2-billion-parameter model on consumer RTX 5090 GPUs. A ~$5,090 run reportedly outperforms Qwen2-1.5B; a ~$6,890 run approaches Qwen2.5-1.5B. Training code, checkpoints, and data are released. The significance is the cost floor: credible small-model pretraining is now within reach of individual research groups and corporate teams without frontier-scale clusters. arXiv →

<!-- graph:start -->
## Connections

**Topics:** [[Infrastructure & Compute]] · [[Model Breakthroughs]]
**Related:** [[2026-08-31-tsinghua-s-pacman-group-releases-puro-2b-a-fully-open-recipe]] · [[2026-09-01-tsinghua-s-pacman-group-publishes-puro-2b-a-full-pretraining]] · [[2026-05-07-new-zaya1-8b-competitive-open-reasoning-model-trained-entire]] · [[2026-09-04-september-4-2026-the-information]] · [[2026-08-12-nvidia-details-serving-alibaba-s-2-4t-parameter-qwen3-8-mode]]
<!-- graph:end -->
