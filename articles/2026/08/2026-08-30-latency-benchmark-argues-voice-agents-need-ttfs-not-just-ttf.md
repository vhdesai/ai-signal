---
article_id: 2026-08-30-latency-benchmark-argues-voice-agents-need-ttfs-not-just-ttf
title: Latency benchmark argues voice agents need TTFS, not just TTFT
date: '2026-08-30'
source: MarkTechPost
url_original: https://www.marktechpost.com/2026/08/30/lowest-latency-inference-apis-for-voice-and-realtime-agents-a-time-to-first-token-ttft-first-benchmark/
url_canonical: https://www.marktechpost.com/2026/08/30/lowest-latency-inference-apis-for-voice-and-realtime-agents-a-time-to-first-token-ttft-first-benchmark/
url_status: ok
digest_source: digests\raw\2026-08-31_060712_Inbox_Daily AI News Digest – August 31,
  2026.md
content_hash: 03a9aed3985840ba34152df46d407e4bb66e8b579cbaebedaf3385d438959140
normalized_title_hash: fe22487c0ea257e7
canonical_url_hash: aa034e2886f7028f
tags: []
entities: []
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-30-time-to-first-token-benchmark-for-voice-and-realtime-agent-i
- 2026-03-30-salesforce-releases-voiceagentrag-316x-faster-retrieval-for
- 2026-04-02-new-salesforce-voiceagentrag-achieves-316x-retrieval-speedup
- 2026-07-06-openai-releases-gpt-realtime-2-1-and-gpt-realtime-2-1-mini-v
- 2026-08-13-cerebras-runs-openai-s-gpt-5-6-sol-at-750-tokens-per-second
embedding_id: 2026-08-30-latency-benchmark-argues-voice-agents-need-ttfs-not-just-ttf
event_name: ''
---

# Latency benchmark argues voice agents need TTFS, not just TTFT

MarkTechPost published a benchmark-driven analysis of inference APIs for voice and realtime agents, arguing that time to first token is insufficient for conversational systems because text-to-speech often needs a complete clause before it can speak. The piece compares LLM first-token latency, output speed, streaming speech-to-text, and text-to-speech layers, framing practical end-to-end voice latency targets around roughly 700ms to 1.2 seconds. For product teams, the takeaway is that model/provider selection should optimize the full turn pipeline rather than a single inference metric. Research Breakthroughs RESEARCHAGENTS

<!-- graph:start -->
## Connections

**Topics:** [[Corporate Moves]]
**Related:** [[2026-08-30-time-to-first-token-benchmark-for-voice-and-realtime-agent-i]] · [[2026-03-30-salesforce-releases-voiceagentrag-316x-faster-retrieval-for]] · [[2026-04-02-new-salesforce-voiceagentrag-achieves-316x-retrieval-speedup]] · [[2026-07-06-openai-releases-gpt-realtime-2-1-and-gpt-realtime-2-1-mini-v]] · [[2026-08-13-cerebras-runs-openai-s-gpt-5-6-sol-at-750-tokens-per-second]]
<!-- graph:end -->
