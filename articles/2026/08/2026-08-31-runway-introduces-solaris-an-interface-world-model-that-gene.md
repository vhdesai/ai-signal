---
article_id: 2026-08-31-runway-introduces-solaris-an-interface-world-model-that-gene
title: Runway Introduces Solaris, an "Interface World Model" That Generates UI Frame
  by Frame
date: '2026-08-31'
source: Runway
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-09-01_081735_Inbox_Daily AI News Digest - September
  1, 2026.md
content_hash: 2fa2d148ae3f349f3c1bc1fd5497b9f902160b6ec37ab452f032d0dc5367f568
normalized_title_hash: 53929b22d3a04e7d
canonical_url_hash: ''
tags:
- Launch
- Hot
entities: []
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-31-runway-unveils-solaris-its-first-interface-world-model
- 2026-09-20-runway-details-real-time-steerable-video-generation-on-its-g
- 2026-06-06-cursor-3-7-ships-design-mode-edit-ui-by-pointing-drawing-or
- 2026-08-29-code-as-world-an-agentic-loop-that-rewrites-real-video-into
- 2026-05-22-replit-ships-visual-editor-layout-controls-and-ai-connectors
embedding_id: 2026-08-31-runway-introduces-solaris-an-interface-world-model-that-gene
event_name: ''
---

# Runway Introduces Solaris, an "Interface World Model" That Generates UI Frame by Frame

Solaris treats a software interface as a generated visual world rather than rendered HTML/CSS/JS — user clicks and drags condition the next generated frame. Runway reports evaluators preferred Solaris output over Claude Opus 5-built UIs in 61% of instruction-following and 71% of natural-behavior judgments. No independent verification has been published. Access is early and request-based. The open question: whether generated interfaces can maintain state and consistency outside curated demos.

<!-- graph:start -->
## Connections

**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-08-31-runway-unveils-solaris-its-first-interface-world-model]] · [[2026-09-20-runway-details-real-time-steerable-video-generation-on-its-g]] · [[2026-06-06-cursor-3-7-ships-design-mode-edit-ui-by-pointing-drawing-or]] · [[2026-08-29-code-as-world-an-agentic-loop-that-rewrites-real-video-into]] · [[2026-05-22-replit-ships-visual-editor-layout-controls-and-ai-connectors]]
<!-- graph:end -->
