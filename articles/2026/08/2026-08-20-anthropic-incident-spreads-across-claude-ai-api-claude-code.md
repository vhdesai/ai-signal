---
article_id: 2026-08-20-anthropic-incident-spreads-across-claude-ai-api-claude-code
title: Anthropic Incident Spreads Across Claude.ai, API, Claude Code and Cowork
date: '2026-08-20'
source: Unite.AI
url_original: https://www.unite.ai/claude-outage-hits-claude-ai-api-claude-code-and-cowork-as-errors-spread-across-models/
url_canonical: https://www.unite.ai/claude-outage-hits-claude-ai-api-claude-code-and-cowork-as-errors-spread-across-models/
url_status: found
digest_source: digests\raw\2026-08-21_061353_Inbox_Daily AI News Digest - August 21,
  2026.md
content_hash: 864df5f7ebbbe77d3ccdadc037a8d025101cbf9a69a5623847f8afce6f2f9c82
normalized_title_hash: 970216c210ca86f7
canonical_url_hash: ecf17af520f6cbfa
tags: []
entities:
- Anthropic
themes:
- datacenter-infrastructure
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-20-anthropic-service-disruption-spreads-across-claude-ai-api-cl
- 2026-08-05-global-anthropic-outage-disrupted-claude-chat-api-and-claude
- 2026-08-03-anthropic-discloses-claude-incidents-in-cybersecurity-evalua
- 2026-08-24-anthropic-resolves-a-multi-model-claude-outage-after-a-month
embedding_id: 2026-08-20-anthropic-incident-spreads-across-claude-ai-api-claude-code
event_name: ''
---

# Anthropic Incident Spreads Across Claude.ai, API, Claude Code and Cowork

Anthropic opened an incident after elevated error rates hit requests across a broad set of Claude models, with impact reaching the consumer app, the API, and its developer and agentic surfaces simultaneously. The blast radius — rather than the duration — is the signal: single-vendor agent dependencies now create correlated downtime across coding, chat, and workflow automation. Enterprises running production agents should validate fallback routing and degradation behavior, not just SLAs. Industry News M&AHOT

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]]
**Topics:** [[Infrastructure & Compute]]
**Related:** [[2026-08-20-anthropic-service-disruption-spreads-across-claude-ai-api-cl]] · [[2026-08-05-global-anthropic-outage-disrupted-claude-chat-api-and-claude]] · [[2026-08-03-anthropic-discloses-claude-incidents-in-cybersecurity-evalua]] · [[2026-08-24-anthropic-resolves-a-multi-model-claude-outage-after-a-month]]
<!-- graph:end -->
