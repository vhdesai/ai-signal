---
article_id: 2026-08-23-who-s-behind-the-new-stealth-model-ox-alpha
title: Who's behind the new 'stealth model' Ox Alpha?
date: '2026-08-23'
source: TechCrunch
url_original: https://techcrunch.com/2026/08/23/whos-behind-the-new-stealth-model-ox-alpha/
url_canonical: https://techcrunch.com/2026/08/23/whos-behind-the-new-stealth-model-ox-alpha/
url_status: ok
digest_source: digests\raw\2026-08-24_060708_Inbox_Daily AI News Digest - August 24,
  2026.md
content_hash: 34208fa7817cecade5f594cefbf82e55318f8231255908ec903617766f837e96
normalized_title_hash: a882954329539eeb
canonical_url_hash: 1538fb9c10b7b42c
tags: []
entities:
- Microsoft
themes:
- model-capabilities
- ma-activity
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-08-23-techcrunch-deep-dive-who-s-behind-the-stealth-model-ox-alpha
related_article_ids: []
embedding_id: 2026-08-23-who-s-behind-the-new-stealth-model-ox-alpha
event_name: ''
---

# Who's behind the new 'stealth model' Ox Alpha?

A free anonymous model called Ox Alpha appeared on OpenRouter on Thursday, August 20, billed as a reasoning model built for coding, sustained agentic work and production workloads, with its provider unnamed during the preview. Stripe CEO Patrick Collison, whose company is acquiring OpenRouter, called it "very impressive." Attribution speculation has moved between Z.ai's GLM family and an unreleased Microsoft MAI model, with no confirmation from any lab. Anonymous preview launches are becoming a standard route for benchmarking a frontier coding model before committing to a brand. VERTICAL AI

<!-- graph:start -->
## Connections

**Entities:** [[Microsoft]]
**Topics:** [[Model Breakthroughs]] · [[M&A Activity]]
**Canonical:** [[2026-08-23-techcrunch-deep-dive-who-s-behind-the-stealth-model-ox-alpha]]
<!-- graph:end -->
