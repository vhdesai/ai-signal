---
article_id: 2026-08-06-apple-proposes-locking-pretrained-weights-to-limit-unauthori
title: Apple proposes locking pretrained weights to limit unauthorized model adaptation
date: '2026-08-06'
source: Apple Machine Learning Research
url_original: https://machinelearning.apple.com/research/locking-pretrained-weights
url_canonical: https://machinelearning.apple.com/research/locking-pretrained-weights
url_status: found
digest_source: digests\raw\2026-08-07_060636_Inbox_Daily AI News Digest - August 7,
  2026.md
content_hash: f6c4865cc5b99c1717cabec4678986c6c8973b5f8a7483c961d3dd66a406fa2a
normalized_title_hash: 9a4bdaada9ac6521
canonical_url_hash: 3911ae4f80259ae6
tags:
- New
entities:
- Apple
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-31-de-restricted-open-weight-models-grow-easier-to-obtain-and-h
- 2026-08-05-apple-research-targets-outlier-token-artifacts-in-diffusion
- 2026-08-03-apple-researchers-study-alignment-methods-for-multimodal-llm
- 2026-08-04-open-weight-ai-models-narrow-the-capability-gap-while-safety
- 2026-04-23-apple-ml-research-releases-pararnn-large-scale-parallelizabl
embedding_id: 2026-08-06-apple-proposes-locking-pretrained-weights-to-limit-unauthori
event_name: ''
---

# Apple proposes locking pretrained weights to limit unauthorized model adaptation

Apple researchers published DLR-Lock, a method that replaces pretrained MLP layers with deep low-rank residual networks to preserve model capabilities while making fine-tuning more costly and difficult. The work targets a core tension in open-weight AI: broader adoption and research value versus the risk that released weights can be adapted for unauthorized uses. It is a technical contribution to model-control debates, showing that governance may increasingly be encoded into architectures as well as licenses.

<!-- graph:start -->
## Connections

**Entities:** [[Apple]]
**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-05-31-de-restricted-open-weight-models-grow-easier-to-obtain-and-h]] · [[2026-08-05-apple-research-targets-outlier-token-artifacts-in-diffusion]] · [[2026-08-03-apple-researchers-study-alignment-methods-for-multimodal-llm]] · [[2026-08-04-open-weight-ai-models-narrow-the-capability-gap-while-safety]] · [[2026-04-23-apple-ml-research-releases-pararnn-large-scale-parallelizabl]]
<!-- graph:end -->
