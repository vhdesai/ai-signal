---
article_id: 2026-08-01-amd-releases-instella-moe-16b-a3b-an-open-mixture-of-experts
title: AMD releases Instella-MoE-16B-A3B, an open mixture-of-experts model trained
  on Instinct GPUs
date: '2026-08-01'
source: MarkTechPost
url_original: https://www.marktechpost.com/2026/08/01/amd-releases-instella-moe-16b-a3b-a-fully-open-mixture-of-experts-llm-with-2-8b-active-parameters-trained-on-instinct-gpus/
url_canonical: https://www.marktechpost.com/2026/08/01/amd-releases-instella-moe-16b-a3b-a-fully-open-mixture-of-experts-llm-with-2-8b-active-parameters-trained-on-instinct-gpus/
url_status: found
digest_source: digests\raw\2026-08-03_060602_Inbox_Daily AI News Digest - August 3,
  2026.md
content_hash: 75e2e0d089a573f9864b21e5c670ae56d1dd8c31beaa58408e2bb7e2677cbc42
normalized_title_hash: e676c4a63bb34d4e
canonical_url_hash: 7e0ab8b360c1e19b
tags:
- New
entities:
- AMD
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-08-01-amd-releases-instella-moe-16b-a3b-a-fully-open-mixture-of-ex
related_article_ids: []
embedding_id: 2026-08-01-amd-releases-instella-moe-16b-a3b-an-open-mixture-of-experts
event_name: ''
---

# AMD releases Instella-MoE-16B-A3B, an open mixture-of-experts model trained on Instinct GPUs

MarkTechPost reported that AMD released Instella-MoE-16B-A3B, a fully open mixture-of-experts language model with 16 billion total parameters and 2.8 billion active parameters. The strategic point is less the model alone than the proof point: AMD is trying to show that competitive open models can be trained and served on its Instinct GPU stack. That matters as enterprises look for alternatives to single-vendor accelerator strategies.

<!-- graph:start -->
## Connections

**Entities:** [[AMD]]
**Topics:** [[Model Breakthroughs]]
**Canonical:** [[2026-08-01-amd-releases-instella-moe-16b-a3b-a-fully-open-mixture-of-ex]]
<!-- graph:end -->
