---
article_id: 2026-08-20-nanoclaw-brings-persistent-ai-agent-teams-to-slack-from-a-si
title: NanoClaw Brings Persistent AI Agent Teams to Slack From a Single Message
date: '2026-08-20'
source: VentureBeat
url_original: https://venturebeat.com/orchestration/nanoclaw-comes-to-slack-letting-you-create-persistent-ai-agent-teams-and-colleagues-from-a-single-message
url_canonical: https://venturebeat.com/orchestration/nanoclaw-comes-to-slack-letting-you-create-persistent-ai-agent-teams-and-colleagues-from-a-single-message
url_status: broken
digest_source: digests\raw\2026-08-21_060810_Inbox_Daily AI News Digest - August 21,
  2026.md
content_hash: 52605e5e1ce7ff678a40c18c63cce8aaf7c9a7e5c08f7b0a49a6008c0a190c95
normalized_title_hash: 2a82312be723b26f
canonical_url_hash: 4e763440b99bc6e6
tags:
- Hot
- New
entities: []
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-07-08-slack-s-slackbot-can-now-pull-your-crm-data-generate-charts
- 2026-08-24-anthropic-s-claude-tag-update-reportedly-lets-its-slack-agen
- 2026-08-20-slack-launches-slack-code-putting-ai-coding-agents-inside-te
- 2026-08-07-study-coordinated-ai-agent-teams-beat-a-solo-frontier-model
- 2026-06-23-anthropic-launches-claude-tag-a-persistent-claude-teammate-i
embedding_id: 2026-08-20-nanoclaw-brings-persistent-ai-agent-teams-to-slack-from-a-si
event_name: ''
---

# NanoClaw Brings Persistent AI Agent Teams to Slack From a Single Message

NanoCo, maker of the open-source agent harness NanoClaw, launched a Slack integration that lets users spin up entire teams of persistent AI agents from a single Slack message, each with its own avatar, memory, permissions and identity. Agents can collaborate in channels and shared Canvases, and can also be reached across Telegram and WhatsApp. The self-hosted agents run on customer infrastructure via Socket Mode, keeping execution inside the enterprise boundary. The feature is free through the Slack Marketplace.

<!-- graph:start -->
## Connections

**Topics:** [[Corporate Moves]]
**Related:** [[2026-07-08-slack-s-slackbot-can-now-pull-your-crm-data-generate-charts]] · [[2026-08-24-anthropic-s-claude-tag-update-reportedly-lets-its-slack-agen]] · [[2026-08-20-slack-launches-slack-code-putting-ai-coding-agents-inside-te]] · [[2026-08-07-study-coordinated-ai-agent-teams-beat-a-solo-frontier-model]] · [[2026-06-23-anthropic-launches-claude-tag-a-persistent-claude-teammate-i]]
<!-- graph:end -->
