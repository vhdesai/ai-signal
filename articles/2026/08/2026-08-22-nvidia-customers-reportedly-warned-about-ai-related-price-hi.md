---
article_id: 2026-08-22-nvidia-customers-reportedly-warned-about-ai-related-price-hi
title: Nvidia customers reportedly warned about AI-related price hikes
date: '2026-08-22'
source: August 22, 2026
url_original: https://www.cnbc.com/2026/08/22/nvidia-customers-reportedly-warned-about-ai-related-price-hikes-.html
url_canonical: https://www.cnbc.com/2026/08/22/nvidia-customers-reportedly-warned-about-ai-related-price-hikes-.html
url_status: found
digest_source: digests\raw\2026-08-24_060708_Inbox_Daily AI News Digest - August 24,
  2026.md
content_hash: 8d785f96439b8004e598ade0d201acade38c241101a2d2a2206404fa38e8c88b
normalized_title_hash: 667527987c7e5282
canonical_url_hash: fb5b25107b0d7bab
tags: []
entities:
- NVIDIA
themes:
- datacenter-infrastructure
- infrastructure-investments
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-23-nvidia-warns-largest-customers-of-15-price-increases-on-ai-s
- 2026-08-22-nvidia-ai-server-prices-to-rise-more-than-15-on-memory-costs
embedding_id: 2026-08-22-nvidia-customers-reportedly-warned-about-ai-related-price-hi
event_name: ''
---

# Nvidia customers reportedly warned about AI-related price hikes

Nvidia has told some of its largest customers that prices for servers containing its AI chips could rise more than 15%, according to Bloomberg. SCMP reported on August 23 that the increases affect systems including Vera Rubin and Grace Blackwell configurations, take effect on systems shipping early next year, and are attributed to component and memory cost inflation. For buyers, this shifts AI infrastructure planning assumptions three days before Nvidia's earnings; memory pricing is now a first-order variable in total cost of ownership. Included as a 24-48 hour item: originated August 22, materially re-reported August 23. Research Breakthroughs CLAIMED

<!-- graph:start -->
## Connections

**Entities:** [[NVIDIA]]
**Topics:** [[Infrastructure & Compute]] · [[Infrastructure Investments]]
**Related:** [[2026-08-23-nvidia-warns-largest-customers-of-15-price-increases-on-ai-s]] · [[2026-08-22-nvidia-ai-server-prices-to-rise-more-than-15-on-memory-costs]]
<!-- graph:end -->
