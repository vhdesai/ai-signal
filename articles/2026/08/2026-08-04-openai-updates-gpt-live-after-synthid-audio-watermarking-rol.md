---
article_id: 2026-08-04-openai-updates-gpt-live-after-synthid-audio-watermarking-rol
title: OpenAI updates GPT-Live after SynthID audio-watermarking rollout
date: '2026-08-04'
source: OpenAI Blog
url_original: https://openai.com/index/introducing-gpt-live/
url_canonical: https://openai.com/index/introducing-gpt-live/
url_status: found
digest_source: digests\raw\2026-08-04_061719_Inbox_Daily AI News Digest - August 4,
  2026.md
content_hash: 5a6536e4f439820ffd5772f2a173b49a76abe36e556975d5e021011629387be6
normalized_title_hash: ac227775ebb34afc
canonical_url_hash: c84f0489248c095a
tags: []
entities:
- OpenAI
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-18-openai-released-three-new-voice-api-models-designed-for-live
- 2026-08-03-openai-details-gpt-live-its-full-duplex-voice-system-for-sub
- 2026-07-08-openai-launches-gpt-live-a-full-duplex-voice-upgrade-for-cha
- 2026-08-06-openai-details-engineering-behind-its-real-time-voice-system
embedding_id: 2026-08-04-openai-updates-gpt-live-after-synthid-audio-watermarking-rol
event_name: ''
---

# OpenAI updates GPT-Live after SynthID audio-watermarking rollout

OpenAI's GPT-Live release page was materially updated in the current window and includes a dated update that supported GPT-Live audio generated through ChatGPT Voice and the OpenAI API now carries SynthID watermarking. GPT-Live is OpenAI's full-duplex voice architecture, built to listen and speak simultaneously while delegating harder reasoning to frontier models in the background. The watermarking component matters because real-time voice AI raises fraud, impersonation, and provenance risks beyond text interfaces. MODEL ECONOMICSOPENAIPRICE PERFORMANCE

<!-- graph:start -->
## Connections

**Entities:** [[OpenAI]]
**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-05-18-openai-released-three-new-voice-api-models-designed-for-live]] · [[2026-08-03-openai-details-gpt-live-its-full-duplex-voice-system-for-sub]] · [[2026-07-08-openai-launches-gpt-live-a-full-duplex-voice-upgrade-for-cha]] · [[2026-08-06-openai-details-engineering-behind-its-real-time-voice-system]]
<!-- graph:end -->
