---
article_id: 2026-08-05-meta-ships-muse-code-a-terminal-coding-agent-built-on-muse-s
title: Meta Ships Muse Code, a Terminal Coding Agent Built on Muse Spark 1.2
date: '2026-08-05'
source: CNBC
url_original: https://www.cnbc.com/2026/08/05/meta-debuts-muse-code-to-take-on-anthropic-and-openai-.html
url_canonical: https://www.cnbc.com/2026/08/05/meta-debuts-muse-code-to-take-on-anthropic-and-openai-.html
url_status: found
digest_source: digests\raw\2026-08-06_061505_Inbox_Daily AI News Digest - August 6,
  2026.md
content_hash: 9c055f9a55b6b7f60d7cef652c9b8eff3f86107598e5da05594a987bd95d7a5f
normalized_title_hash: db4c124ae4b8d761
canonical_url_hash: aaafddabd3008b9a
tags:
- Launch
- Hot
entities:
- Anthropic
- Meta
- OpenAI
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-08-05-meta-debuts-muse-code-and-muse-spark-1-2-entering-the-coding
related_article_ids:
- 2026-08-05-meta-ships-muse-code-and-muse-spark-1-2-entering-the-coding
embedding_id: 2026-08-05-meta-ships-muse-code-a-terminal-coding-agent-built-on-muse-s
event_name: ''
---

# Meta Ships Muse Code, a Terminal Coding Agent Built on Muse Spark 1.2

Meta released Muse Code, its first coding agent, positioned as a lower-cost challenger to Anthropic's and OpenAI's developer offerings. The product runs persistent background agents with repository-scale execution, multimodal understanding, and built-in verification. Meta is reportedly pushing thousands of its own engineers onto an internal coding agent to generate training signal — the same self-as-customer pattern visible across the sector this week. ________________________________ RESEARCH

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Meta]] · [[OpenAI]]
**Topics:** [[Model Breakthroughs]]
**Canonical:** [[2026-08-05-meta-debuts-muse-code-and-muse-spark-1-2-entering-the-coding]]
**Related:** [[2026-08-05-meta-ships-muse-code-and-muse-spark-1-2-entering-the-coding]]
<!-- graph:end -->
