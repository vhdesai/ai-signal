---
article_id: 2026-08-21-anthropic-to-let-enterprises-hold-required-retention-data-on
title: Anthropic to let enterprises hold required retention data on their own cloud
date: '2026-08-21'
source: Yahoo Finance
url_original: https://finance.yahoo.com/technology/ai/articles/anthropic-let-enterprise-customers-store-125515386.html
url_canonical: https://finance.yahoo.com/technology/ai/articles/anthropic-let-enterprise-customers-store-125515386.html
url_status: ok
digest_source: digests\raw\2026-08-22_060308_Inbox_Daily AI News Digest - August 22,
  2026.md
content_hash: 8a9008354eccb202dd9a87a7ca71f258dd25932c40691a4d20dd4e140f9822d3
normalized_title_hash: 75fc64603a8840bb
canonical_url_hash: 607bd0c876eb7b53
tags:
- Breaking
entities:
- Anthropic
- OpenAI
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-20-anthropic-reverses-course-on-enterprise-data-retention-after
- 2026-08-19-openai-launches-private-safety-processing-to-one-up-anthropi
- 2026-08-20-openai-is-gaining-on-anthropic-with-business-users-new-data
embedding_id: 2026-08-21-anthropic-to-let-enterprises-hold-required-retention-data-on
event_name: ''
---

# Anthropic to let enterprises hold required retention data on their own cloud

Anthropic will let enterprise customers store the mandated 30-day retention data on their own cloud infrastructure rather than Anthropic's systems. The 30-day requirement — introduced in June to guard against cyber misuse — remains, and the change applies to the Fable and Mythos model families and future frontier models. It follows OpenAI's debut of a misuse-detection system that does not retain customer data, confirming that data-control terms are now a primary enterprise differentiator. SAFETY

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[OpenAI]]
**Topics:** [[Policy & Regulation]]
**Related:** [[2026-08-20-anthropic-reverses-course-on-enterprise-data-retention-after]] · [[2026-08-19-openai-launches-private-safety-processing-to-one-up-anthropi]] · [[2026-08-20-openai-is-gaining-on-anthropic-with-business-users-new-data]]
<!-- graph:end -->
