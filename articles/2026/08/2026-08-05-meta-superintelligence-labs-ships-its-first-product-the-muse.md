---
article_id: 2026-08-05-meta-superintelligence-labs-ships-its-first-product-the-muse
title: Meta Superintelligence Labs ships its first product, the “Muse Code” agent
date: '2026-08-05'
source: VentureBeat, InfoWorld, Unite.AI
url_original: https://venturebeat.com/orchestration/meta-enters-the-ai-coding-wars-with-muse-spark-1-2-and-muse-code-with-persistent-async-background-agents
url_canonical: https://venturebeat.com/orchestration/meta-enters-the-ai-coding-wars-with-muse-spark-1-2-and-muse-code-with-persistent-async-background-agents
url_status: broken
digest_source: digests\raw\2026-08-06_061306_Inbox_Daily AI News Digest - August 6,
  2026.md
content_hash: 61c30e6a84e878a4fd32400c82a9b2c6a3e19cbe7ecb96df95558bfb9677ba29
normalized_title_hash: fc5bed9258693258
canonical_url_hash: 1547e15d51656207
tags:
- Hot
entities:
- Meta
- Scale AI
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-08-05-meta-launches-muse-code-an-ai-coding-agent-for-large-codebas
related_article_ids:
- 2026-08-07-meta-launches-muse-code-an-ai-coding-agent-to-rival-openai-a
embedding_id: 2026-08-05-meta-superintelligence-labs-ships-its-first-product-the-muse
event_name: ''
---

# Meta Superintelligence Labs ships its first product, the “Muse Code” agent

Alexandr Wang's Meta Superintelligence Labs, assembled after Meta's roughly $14.3B Scale AI deal, shipped Muse Code, a terminal-based coding agent with persistent async background agents and a replay-exact event log. It runs on Muse Spark 1.2, a model co-trained with the harness on repository-scale work, and vendor benchmarks claim 82.9% on Terminal-Bench 2.1, trailing only Claude Opus 5 at 86.7%. The strategic hook is a cut-rate contributor tier at roughly $0.10/$0.20 per million tokens where developers opt into having prompts and completions used to train Meta's models. InfoWorld notes analysts do not yet see the approach as differentiated from rivals, and the benchmarks remain vendor-reported.

<!-- graph:start -->
## Connections

**Entities:** [[Meta]] · [[Scale AI]]
**Topics:** [[Corporate Moves]]
**Canonical:** [[2026-08-05-meta-launches-muse-code-an-ai-coding-agent-for-large-codebas]]
**Related:** [[2026-08-07-meta-launches-muse-code-an-ai-coding-agent-to-rival-openai-a]]
<!-- graph:end -->
