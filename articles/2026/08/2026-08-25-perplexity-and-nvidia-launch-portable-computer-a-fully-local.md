---
article_id: 2026-08-25-perplexity-and-nvidia-launch-portable-computer-a-fully-local
title: Perplexity and Nvidia Launch "Portable Computer," a Fully Local AI Agent With
  Zero Token Costs
date: '2026-08-25'
source: VentureBeat
url_original: https://venturebeat.com/infrastructure/perplexity-partners-with-nvidia-to-launch-portable-computer-a-fully-local-ai-agent-with-zero-token-costs
url_canonical: https://venturebeat.com/infrastructure/perplexity-partners-with-nvidia-to-launch-portable-computer-a-fully-local-ai-agent-with-zero-token-costs
url_status: found
digest_source: digests\raw\2026-08-26_062133_Inbox_Daily AI News Digest - August 26,
  2026.md
content_hash: 8220071b2c482c216eab471c8f7cdc873e63e6731c7ac35a48fa7e5eba446fae
normalized_title_hash: aabc4ff1c9892ffc
canonical_url_hash: c88f316e61d33f06
tags:
- Launch
entities:
- NVIDIA
- Perplexity
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-08-25-perplexity-partners-with-nvidia-to-launch-portable-computer
- 2026-08-25-perplexity-ships-portable-computer-on-nvidia-dgx-spark
- 2026-08-11-nvidia-switchyard-mid-task-model-router-cuts-agent-costs-to
- 2026-08-24-nvidia-discusses-perplexity-investment-at-30-billion-plus-va
embedding_id: 2026-08-25-perplexity-and-nvidia-launch-portable-computer-a-fully-local
event_name: ''
---

# Perplexity and Nvidia Launch "Portable Computer," a Fully Local AI Agent With Zero Token Costs

Perplexity, partnering with Nvidia, launched Portable Computer — an agent platform that runs entirely on-device on Nvidia DGX Spark and RTX-powered Linux machines, with no per-token cloud fees. Cloud routing is user-gated rather than default, positioning the product for privacy-sensitive and cost-sensitive workloads. If local agent stacks mature, the marginal-cost argument for cloud inference weakens for a meaningful class of enterprise tasks. ________________________________ ENTERPRISE

<!-- graph:start -->
## Connections

**Entities:** [[NVIDIA]] · [[Perplexity]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-08-25-perplexity-partners-with-nvidia-to-launch-portable-computer]] · [[2026-08-25-perplexity-ships-portable-computer-on-nvidia-dgx-spark]] · [[2026-08-11-nvidia-switchyard-mid-task-model-router-cuts-agent-costs-to]] · [[2026-08-24-nvidia-discusses-perplexity-investment-at-30-billion-plus-va]]
<!-- graph:end -->
