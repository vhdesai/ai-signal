---
article_id: 2026-06-02-openai-extends-codex-to-business-with-plugins-sites-and-anno
title: OpenAI Extends Codex to Business with Plugins, Sites, and Annotations
date: '2026-06-02'
source: 9to5Mac; TechCrunch; VentureBeat; Quartz
url_original: https://9to5mac.com/2026/06/02/openai-putting-codex-inside-chatgpt-app-everywhere-releasing-6-business-plugins/
url_canonical: https://9to5mac.com/2026/06/02/openai-putting-codex-inside-chatgpt-app-everywhere-releasing-6-business-plugins/
url_status: ok
digest_source: digests\raw\2026-06-03_083329_Inbox_Daily AI News Digest – June 3,
  2026 (Updated).md
content_hash: 423a7522887d657b18baaddaf8ce5f3e7e3f4f584433ae3141508558d6a3e8b5
normalized_title_hash: 46c0bed6fad07463
canonical_url_hash: becd56a958240f61
tags:
- Hot
- New
entities:
- OpenAI
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-06-02-openai-expands-codex-agent-to-finance-banking-and-sales
- 2026-05-14-openai-codex-work-from-anywhere-expansion
- 2026-05-18-openai-expands-codex-hybrid-on-prem-via-dell-launches-chatgp
- 2026-05-14-openai-brings-codex-to-mobile-extending-agentic-coding-beyon
- 2026-05-14-openai-expands-codex-platform-windows-sandbox-mobile-access
embedding_id: 2026-06-02-openai-extends-codex-to-business-with-plugins-sites-and-anno
event_name: ''
---

# OpenAI Extends Codex to Business with Plugins, Sites, and Annotations

OpenAI announced six new Codex agent plugins, an annotations feature, and a “Sites” feature, and said it will bring Codex functionality into the ChatGPT app broadly “in the next few weeks.” The intent is to convert Codex from a developer-only agentic coding tool into a general enterprise productivity surface riding ChatGPT’s existing footprint. For enterprises already standardized on ChatGPT, this lowers the adoption barrier to agentic workflows.

<!-- graph:start -->
## Connections

**Entities:** [[OpenAI]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-06-02-openai-expands-codex-agent-to-finance-banking-and-sales]] · [[2026-05-14-openai-codex-work-from-anywhere-expansion]] · [[2026-05-18-openai-expands-codex-hybrid-on-prem-via-dell-launches-chatgp]] · [[2026-05-14-openai-brings-codex-to-mobile-extending-agentic-coding-beyon]] · [[2026-05-14-openai-expands-codex-platform-windows-sandbox-mobile-access]]
<!-- graph:end -->
