---
article_id: 2026-06-28-prompt-injection-named-the-top-enterprise-ai-attack-vector-a
title: Prompt injection named the top enterprise-AI attack vector as agents and RAG
  widen the surface
date: '2026-06-28'
source: VentureBeat
url_original: https://venturebeat.com/security/prompt-injection-is-exploiting-enterprise-ais-biggest-design-flaws-by-targeting-agents-rag-pipelines-and-model-routers
url_canonical: https://venturebeat.com/security/prompt-injection-is-exploiting-enterprise-ais-biggest-design-flaws-by-targeting-agents-rag-pipelines-and-model-routers
url_status: broken
digest_source: digests\raw\2026-06-29_062122_Inbox_Daily AI News Digest - June 29,
  2026.md
content_hash: 6fdabc30e4978037cf2fcdfa738117c92e414910aa6b95499768fe9ca5d36beb
normalized_title_hash: 9c58508de8035fa5
canonical_url_hash: 6650f116c8aeac1b
tags: []
entities: []
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-06-29-venturebeat-details-how-prompt-injection-remains-the-top-ent
- 2026-06-26-this-paper-examines-prompt-injection-attacks-against-llm-bas
- 2026-05-24-venturebeat-ai-agents-are-creating-an-untracked-class-of-pro
- 2026-05-10-new-arxiv-may-2026-1-200-ai-papers-agentic-reputation-system
- 2026-06-05-microsoft-identifies-seven-new-attack-vectors-against-ai-age
embedding_id: 2026-06-28-prompt-injection-named-the-top-enterprise-ai-attack-vector-a
event_name: ''
---

# Prompt injection named the top enterprise-AI attack vector as agents and RAG widen the surface

VentureBeat details how prompt injection — ranked LLM01 in the OWASP LLM Top 10 — has evolved beyond chatbots to target AI agents, RAG pipelines, model routers and long-term memory, with CrowdStrike documenting malicious-prompt injections across 90+ organizations in 2025 ("prompts are the new malware"). Newer variants include RAG supply-chain poisoning, agent hijacking, context-overflow and memory poisoning. As enterprises wire LLMs into workflows that send email and execute code, the piece argues models must be treated as untrusted interpreters, not trusted automation.

<!-- graph:start -->
## Connections

**Topics:** [[Policy & Regulation]]
**Related:** [[2026-06-29-venturebeat-details-how-prompt-injection-remains-the-top-ent]] · [[2026-06-26-this-paper-examines-prompt-injection-attacks-against-llm-bas]] · [[2026-05-24-venturebeat-ai-agents-are-creating-an-untracked-class-of-pro]] · [[2026-05-10-new-arxiv-may-2026-1-200-ai-papers-agentic-reputation-system]] · [[2026-06-05-microsoft-identifies-seven-new-attack-vectors-against-ai-age]]
<!-- graph:end -->
