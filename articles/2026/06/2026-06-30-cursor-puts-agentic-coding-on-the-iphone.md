---
article_id: 2026-06-30-cursor-puts-agentic-coding-on-the-iphone
title: Cursor puts agentic coding on the iPhone
date: '2026-06-30'
source: TechRepublic
url_original: https://www.techrepublic.com/article/news-spacex-cursor-ai-coding-agents-iphone/
url_canonical: https://www.techrepublic.com/article/news-spacex-cursor-ai-coding-agents-iphone/
url_status: broken
digest_source: digests\raw\2026-07-01_062608_Inbox_Daily AI News Digest - July 1,
  2026.md
content_hash: fae5efc869c3d13df2add8136303bdd1879576e04421602f03cd5453358fb613
normalized_title_hash: 0c1d5db7e0a11fe3
canonical_url_hash: c87b140ca1013305
tags: []
entities:
- Anthropic
- OpenAI
themes:
- datacenter-infrastructure
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-06-29-cursor-launches-a-mobile-app-to-drive-coding-agents-from-you
- 2026-05-26-replit-closes-400m-round-at-9b-valuation-as-ai-coding-wars-i
- 2026-05-15-cursor-launches-hiring-spree-after-spacex-deal
- 2026-05-22-xai-spacex-secures-60b-option-to-acquire-cursor-explores-thr
- 2026-04-23-cursor-and-replit-push-deeper-agentic-coding-workflows
embedding_id: 2026-06-30-cursor-puts-agentic-coding-on-the-iphone
event_name: ''
---

# Cursor puts agentic coding on the iPhone

Cursor — the AI coding platform whose parent, Anysphere, agreed to a roughly $60B all-stock acquisition by SpaceX — released a public-beta iPhone and iPad app that lets paid users launch, monitor, review, and merge the work of cloud coding agents from mobile. Agents run in isolated cloud VMs and keep working with the app closed, surfacing progress via push notifications and lock-screen Live Activities. The release, which followed the app's June 29 debut, underscores a broader shift from writing code to supervising autonomous agents, joining mobile offerings from Anthropic and OpenAI. No Android version has been announced. Infrastructure B INFRA FUNDING

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[OpenAI]]
**Topics:** [[Infrastructure & Compute]] · [[Corporate Moves]]
**Related:** [[2026-06-29-cursor-launches-a-mobile-app-to-drive-coding-agents-from-you]] · [[2026-05-26-replit-closes-400m-round-at-9b-valuation-as-ai-coding-wars-i]] · [[2026-05-15-cursor-launches-hiring-spree-after-spacex-deal]] · [[2026-05-22-xai-spacex-secures-60b-option-to-acquire-cursor-explores-thr]] · [[2026-04-23-cursor-and-replit-push-deeper-agentic-coding-workflows]]
<!-- graph:end -->
