---
article_id: 2026-06-02-microsoft-launches-scout-agent-agent-365-governance-and-surf
title: Microsoft Launches Scout Agent, Agent 365 Governance, and Surface RTX Spark
  Dev Box
date: '2026-06-02'
source: Microsoft; TechCrunch; Wired; The Verge; Gizbot
url_original: https://techcrunch.com/2026/06/02/microsoft-launches-scout-an-openclaw-inspired-personal-assistant/
url_canonical: https://techcrunch.com/2026/06/02/microsoft-launches-scout-an-openclaw-inspired-personal-assistant/
url_status: found
digest_source: digests\raw\2026-06-04_072143_Inbox_Daily AI News Digest – June 4,
  2026 (Updated).md
content_hash: cf452a908607beec8aee7b849bb361f1f2fad471d0802d42a7483dd6d518cd2c
normalized_title_hash: 59fb7ab719c9490f
canonical_url_hash: 799e29949be738ac
tags:
- Breaking
- Hot
entities:
- Microsoft
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- event-microsoft-build-2026-microsoft-build-2026-agents-agent-platforms-and-agent-lifecy
- 2026-06-02-microsoft-build-2026-scout-project-solara-and-frontier-tunin
- event-microsoft-build-2026-microsoft-build-2026-windows-local-agents-and-developer-devi
- 2026-05-19-gemini-spark-google-s-24-7-personal-ai-agent-launches-next-w
embedding_id: 2026-06-02-microsoft-launches-scout-agent-agent-365-governance-and-surf
event_name: ''
---

# Microsoft Launches Scout Agent, Agent 365 Governance, and Surface RTX Spark Dev Box

Scout is the first “Autopilots”-class always-on agent, built on OpenClaw and grounded in Work IQ across M365. Each agent gets its own Entra identity; Agent 365 and Microsoft IQ provide governance. The Surface RTX Spark Dev Box delivers up to 1 petaflop with 128GB unified memory, running 120B-parameter models locally. Maia 200 is live in Iowa and Arizona data centers. Scout is available immediately to Frontier-program customers.

<!-- graph:start -->
## Connections

**Entities:** [[Microsoft]]
**Topics:** [[Corporate Moves]]
**Related:** [[event-microsoft-build-2026-microsoft-build-2026-agents-agent-platforms-and-agent-lifecy]] · [[2026-06-02-microsoft-build-2026-scout-project-solara-and-frontier-tunin]] · [[event-microsoft-build-2026-microsoft-build-2026-windows-local-agents-and-developer-devi]] · [[2026-05-19-gemini-spark-google-s-24-7-personal-ai-agent-launches-next-w]]
<!-- graph:end -->
