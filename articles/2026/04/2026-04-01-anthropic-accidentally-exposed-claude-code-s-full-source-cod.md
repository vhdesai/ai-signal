---
article_id: 2026-04-01-anthropic-accidentally-exposed-claude-code-s-full-source-cod
title: 'Anthropic accidentally exposed Claude Code''s full source code — including
  system prompt architecture and model-steering techniques — then triggered a secondary
  incident by mass-removing GitHub repos in cleanup, which TechCrunch says was itself
  an error. Someone cracked the code signing system within 24 hours. No hack involved
  — human error. Marc Andreessen: both the Anthropic and Mercor incidents mark the
  end of the AI industry''s "we''ll lock it up" approach to model security. Two simultaneous
  AI IP breaches in one day has made model security an urgent board-level issue.'
date: '2026-04-01'
source: CNBC
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-04-02_082458_Inbox_Daily AI News Digest – April 2,
  2026.md
content_hash: 9029c099b2a44641b91a08e3eefcd467d821ae93a49f13950110cc9407f03372
normalized_title_hash: 8b87bbf143a2682c
canonical_url_hash: ''
tags: []
entities:
- Anthropic
- Google
- Microsoft
- NVIDIA
themes:
- model-capabilities
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-04-11-anthropic-launched-project-glasswing-partnering-with-aws-app
- 2026-06-06-anthropic-suffers-multi-model-claude-outage
- 2026-06-05-anthropic-faces-multi-model-claude-outage-notion-disables-an
- 2026-05-01-pentagon-awards-il6-il7-ai-contracts-to-8-firms-anthropic-ex
- 2026-06-02-anthropic-expands-glasswing-to-200-partners-maps-year-of-ai
embedding_id: 2026-04-01-anthropic-accidentally-exposed-claude-code-s-full-source-cod
event_name: ''
---

# Anthropic accidentally exposed Claude Code's full source code — including system prompt architecture and model-steering techniques — then triggered a secondary incident by mass-removing GitHub repos in cleanup, which TechCrunch says was itself an error. Someone cracked the code signing system within 24 hours. No hack involved — human error. Marc Andreessen: both the Anthropic and Mercor incidents mark the end of the AI industry's "we'll lock it up" approach to model security. Two simultaneous AI IP breaches in one day has made model security an urgent board-level issue.

IRGC Threatens 18 U.S. Tech Firms Including Nvidia, Microsoft & Google as "Legitimate Military Targets"

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Google]] · [[Microsoft]] · [[NVIDIA]]
**Topics:** [[Model Breakthroughs]] · [[Corporate Moves]]
**Related:** [[2026-04-11-anthropic-launched-project-glasswing-partnering-with-aws-app]] · [[2026-06-06-anthropic-suffers-multi-model-claude-outage]] · [[2026-06-05-anthropic-faces-multi-model-claude-outage-notion-disables-an]] · [[2026-05-01-pentagon-awards-il6-il7-ai-contracts-to-8-firms-anthropic-ex]] · [[2026-06-02-anthropic-expands-glasswing-to-200-partners-maps-year-of-ai]]
<!-- graph:end -->
