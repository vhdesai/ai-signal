---
article_id: 2026-04-16-anthropic-has-begun-requiring-government-id-verification-via
title: Anthropic has begun requiring government ID verification (via Persona) before
  allowing subscriptions
date: '2026-04-16'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-04-16_075833_Inbox_Daily AI News Digest – April 16,
  2026.md
content_hash: 12697dc373ba35133bfdacca388f7ec757fc49ead20575404f356b0d0727d12b
normalized_title_hash: 56a6e0fc67af5922
canonical_url_hash: ''
tags: []
entities:
- Anthropic
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-06-22-anthropic-says-claude-may-ask-users-to-verify-their-governme
- 2026-08-12-anthropic-s-claude-watermarking-draws-user-backlash-and-comp
- 2026-08-03-anthropic-discloses-claude-incidents-in-cybersecurity-evalua
- 2026-08-05-uk-ai-security-institute-anthropic-and-openai-agents-took-un
- 2026-08-14-anthropic-details-claude-text-watermarking-under-eu-ai-act
embedding_id: 2026-04-16-anthropic-has-begun-requiring-government-id-verification-via
event_name: ''
---

# Anthropic has begun requiring government ID verification (via Persona) before allowing subscriptions

Anthropic has begun requiring government ID verification (via Persona) before allowing subscriptions. The move went viral on X, with critics framing it as a competitive gift to ChatGPT and Gemini, neither of which requires identity verification. Combined with the Heppner ruling, the era of "AI as private confidant" faces new challenges.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]]
**Topics:** [[Policy & Regulation]]
**Related:** [[2026-06-22-anthropic-says-claude-may-ask-users-to-verify-their-governme]] · [[2026-08-12-anthropic-s-claude-watermarking-draws-user-backlash-and-comp]] · [[2026-08-03-anthropic-discloses-claude-incidents-in-cybersecurity-evalua]] · [[2026-08-05-uk-ai-security-institute-anthropic-and-openai-agents-took-un]] · [[2026-08-14-anthropic-details-claude-text-watermarking-under-eu-ai-act]]
<!-- graph:end -->
