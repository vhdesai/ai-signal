---
article_id: 2026-04-04-mercor-the-10b-ai-startup-serving-anthropic-openai-and-meta
title: Mercor, the $10B AI startup serving Anthropic, OpenAI, and Meta, confirmed
  a breach via a supply chain attack on…
date: '2026-04-04'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-04-04_140800_Inbox_Daily AI News Digest – April 4,
  2026.md
content_hash: 6d4e9bad0b0e1c29efa2a1a784eeb5ed6726c20e0304928ae6001fd447fa720f
normalized_title_hash: a4b9169651fd34ac
canonical_url_hash: ''
tags: []
entities:
- Anthropic
- Meta
- OpenAI
themes:
- policy-regulation
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-04-06-a-supply-chain-security-breach-at-mercor-a-platform-widely-u
- 2026-04-02-breaking-mercor-and-litellm-breach-exposes-4tb-of-data-mar-3
- 2026-04-23-contractors-sue-mercor-10b-ai-data-labeling-startup-over-bre
- 2026-04-01-on-march-24-threat-actor-teampcp-executed-a-sophisticated-su
- 2026-04-03-anthropic-is-in-damage-control-mode-after-source-code-for-it
embedding_id: 2026-04-04-mercor-the-10b-ai-startup-serving-anthropic-openai-and-meta
event_name: ''
---

# Mercor, the $10B AI startup serving Anthropic, OpenAI, and Meta, confirmed a breach via a supply chain attack on…

Mercor, the $10B AI startup serving Anthropic, OpenAI, and Meta, confirmed a breach via a supply chain attack on LiteLLM—a widely-used open-source AI orchestration library downloaded millions of times per day. Threat group TeamPCP planted credential-harvesting code before it was identified and removed. Lapsus$ has also claimed involvement. Unconfirmed reports suggest training datasets and proprietary AI project details may have been accessed. Enterprises relying on open-source AI libraries should audit dependencies immediately and review access controls for LiteLLM integrations.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Meta]] · [[OpenAI]]
**Topics:** [[Policy & Regulation]]
**Related:** [[2026-04-06-a-supply-chain-security-breach-at-mercor-a-platform-widely-u]] · [[2026-04-02-breaking-mercor-and-litellm-breach-exposes-4tb-of-data-mar-3]] · [[2026-04-23-contractors-sue-mercor-10b-ai-data-labeling-startup-over-bre]] · [[2026-04-01-on-march-24-threat-actor-teampcp-executed-a-sophisticated-su]] · [[2026-04-03-anthropic-is-in-damage-control-mode-after-source-code-for-it]]
<!-- graph:end -->
