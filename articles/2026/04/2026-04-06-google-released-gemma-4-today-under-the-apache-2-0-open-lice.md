---
article_id: 2026-04-06-google-released-gemma-4-today-under-the-apache-2-0-open-lice
title: Google released Gemma 4 today under the Apache 2.0 open license, achieving
  89.2% on the AIME mathematics benchmark —…
date: '2026-04-06'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-04-06_070950_Inbox_Daily AI News Digest – April 6,
  2026.md
content_hash: 3b289bb26b5a8b8c0027c85c8236d42fa055cd0cb9808476beb68d1c647bb19d
normalized_title_hash: cb5cda62cfc97de2
canonical_url_hash: ''
tags: []
entities:
- Google
- Meta
themes:
- datacenter-infrastructure
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-04-04-google-released-gemma-4-in-four-sizes-e2b-e4b-26b-moe-and-31
- 2026-04-03-google-deepmind-released-gemma-4-a-family-of-four-open-weigh
- 2026-04-10-analysis-of-march-april-2026-benchmark-results-shows-open-we
- 2026-06-03-google-releases-gemma-4-12b-sized-to-run-on-a-16gb-laptop
- 2026-06-03-google-releases-gemma-4-12b-sized-for-a-16gb-laptop
embedding_id: 2026-04-06-google-released-gemma-4-today-under-the-apache-2-0-open-lice
event_name: ''
---

# Google released Gemma 4 today under the Apache 2.0 open license, achieving 89.2% on the AIME mathematics benchmark —…

Google released Gemma 4 today under the Apache 2.0 open license, achieving 89.2% on the AIME mathematics benchmark — placing it third among all open models worldwide. The model is designed to run on consumer-grade GPUs, dramatically lowering the barrier for enterprise deployment. This is Google's most competitive open-weight release to date and signals a direct challenge to Meta's Llama series. Availability is immediate via Hugging Face and Google AI Studio.

<!-- graph:start -->
## Connections

**Entities:** [[Google]] · [[Meta]]
**Topics:** [[Infrastructure & Compute]] · [[Model Breakthroughs]]
**Related:** [[2026-04-04-google-released-gemma-4-in-four-sizes-e2b-e4b-26b-moe-and-31]] · [[2026-04-03-google-deepmind-released-gemma-4-a-family-of-four-open-weigh]] · [[2026-04-10-analysis-of-march-april-2026-benchmark-results-shows-open-we]] · [[2026-06-03-google-releases-gemma-4-12b-sized-to-run-on-a-16gb-laptop]] · [[2026-06-03-google-releases-gemma-4-12b-sized-for-a-16gb-laptop]]
<!-- graph:end -->
