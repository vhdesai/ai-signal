---
article_id: 2026-04-22-an-internal-model-selection-menu-inside-openai-s-codex-platf
title: An internal model selection menu inside OpenAI's Codex platform briefly exposed
  what appears to be a GPT-5.5 family of models before being pulled. Developers who
  captured screenshots reported faster code generation and improved token efficiency.
  The presence of multiple entries under the GPT-5.5 umbrella suggests a tiered lineup
  — mirroring OpenAI's earlier GPT-4 rollout strategy. OpenAI has not made an official
  announcement, but the leak signals an imminent release.
date: '2026-04-22'
source: 🔥 Hot Anthropic
url_original: null
url_canonical: https://developers.openai.com/codex/models
url_status: repaired
digest_source: digests\raw\2026-04-22_071302_Inbox_Daily AI News Digest – April 22,
  2026.md
content_hash: 68a25fd77fde7f6960b7b069876848cf28197ffc9b9eda911f7e752e4c679ffd
normalized_title_hash: 93c54e2301303b60
canonical_url_hash: ''
tags: []
entities:
- Anthropic
- OpenAI
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-04-22-model-releases-previews
- 2026-04-15-model-releases
- 2026-04-30-hotopenai-makes-gpt-5-5-cyber-available-to-federal-cyber-def
- 2026-04-23-
- 2026-05-07-newopenai-gpt-5-5-cyber-rolls-out-to-vetted-security-teams
embedding_id: 2026-04-22-an-internal-model-selection-menu-inside-openai-s-codex-platf
event_name: ''
---

# An internal model selection menu inside OpenAI's Codex platform briefly exposed what appears to be a GPT-5.5 family of models before being pulled. Developers who captured screenshots reported faster code generation and improved token efficiency. The presence of multiple entries under the GPT-5.5 umbrella suggests a tiered lineup — mirroring OpenAI's earlier GPT-4 rollout strategy. OpenAI has not made an official announcement, but the leak signals an imminent release.

Anthropic Investigates Unauthorized Access to Unreleased "Claude Mythos" Model

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[OpenAI]]
**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-04-22-model-releases-previews]] · [[2026-04-15-model-releases]] · [[2026-04-30-hotopenai-makes-gpt-5-5-cyber-available-to-federal-cyber-def]] · [[2026-04-23-]] · [[2026-05-07-newopenai-gpt-5-5-cyber-rolls-out-to-vetted-security-teams]]
<!-- graph:end -->
