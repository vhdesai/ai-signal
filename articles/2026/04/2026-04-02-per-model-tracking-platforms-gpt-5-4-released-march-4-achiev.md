---
article_id: 2026-04-02-per-model-tracking-platforms-gpt-5-4-released-march-4-achiev
title: Per model tracking platforms, GPT-5.4 (released March 4) achieves 0.9 GPQA;
  Mistral Small 4 (March 15) is open source…
date: '2026-04-02'
source: Daily AI News Digest
url_original: null
url_canonical: https://www.arturmarkus.com/12-ai-models-launched-in-one-week-during-march-10-16-2026-openai-google-xai-anthropic-mistral-and-cursor-compress-developer-selection-cycles-to-monthly-as-frontier-model-releases-pile-up/
url_status: repaired
digest_source: digests\raw\2026-04-02_080231_Inbox_Daily AI News Digest – April 2,
  2026.md
content_hash: ab068fc5142bcfdd9a2b70b230e52e19d3da86a6af93536c24ebb291a50afd66
normalized_title_hash: 1d3ae9b46a0bc979
canonical_url_hash: ''
tags: []
entities:
- Huawei
- Mistral
- NVIDIA
themes:
- datacenter-infrastructure
- model-capabilities
cross_cutting_topics:
- china-compete
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-16-gpt-5-4-pro-openai-holds-the-top-spot-on-gpqa-diamond-gradua
- 2026-04-11-deepseek-has-confirmed-its-v4-model-is-targeting-a-late-apri
- 2026-08-14-z-ai-unveils-glm-5-3-an-open-weights-model-with-near-frontie
- 2026-05-04-deepseek-v4-three-chinese-labs-drop-open-weights-frontier-mo
- 2026-04-23-
embedding_id: 2026-04-02-per-model-tracking-platforms-gpt-5-4-released-march-4-achiev
event_name: ''
---

# Per model tracking platforms, GPT-5.4 (released March 4) achieves 0.9 GPQA; Mistral Small 4 (March 15) is open source…

Per model tracking platforms, GPT-5.4 (released March 4) achieves 0.9 GPQA; Mistral Small 4 (March 15) is open source at 0.7 GPQA; Nvidia's Nemotron 3 Super 120B (March 10) hits 0.8 GPQA with open-source weights. Claude Sonnet 4.6 (February 17) offers near-Opus performance with Agent Teams support (orchestrating 2–16 instances) at 80.8% SWE-bench Verified. Zhipu AI's GLM-5 (February 11) — trained entirely on Huawei Ascend chips without Nvidia — achieved a #1 HLE score of 50.4% and a 1.2% hallucination rate, at 136x lower cost than Claude Opus 4.5.

<!-- graph:start -->
## Connections

**Entities:** [[Huawei]] · [[Mistral]] · [[NVIDIA]]
**Topics:** [[Infrastructure & Compute]] · [[Model Breakthroughs]] · [[Global AI Race]]
**Related:** [[2026-05-16-gpt-5-4-pro-openai-holds-the-top-spot-on-gpqa-diamond-gradua]] · [[2026-04-11-deepseek-has-confirmed-its-v4-model-is-targeting-a-late-apri]] · [[2026-08-14-z-ai-unveils-glm-5-3-an-open-weights-model-with-near-frontie]] · [[2026-05-04-deepseek-v4-three-chinese-labs-drop-open-weights-frontier-mo]] · [[2026-04-23-]]
<!-- graph:end -->
