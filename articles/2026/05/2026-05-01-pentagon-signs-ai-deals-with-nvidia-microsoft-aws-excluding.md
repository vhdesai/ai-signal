---
article_id: 2026-05-01-pentagon-signs-ai-deals-with-nvidia-microsoft-aws-excluding
title: Pentagon Signs AI Deals with Nvidia, Microsoft, AWS — Excluding Anthropic
date: '2026-05-01'
source: TechCrunch
url_original: null
url_canonical: https://techcrunch.com/2026/05/01/pentagon-inks-deals-with-nvidia-microsoft-and-aws-to-deploy-ai-on-classified-networks/
url_status: repaired
digest_source: digests\raw\2026-05-15_071213_Inbox_Daily AI News Digest – May 15,
  2026.md
content_hash: 6004e1e423cfb45693479944cc9de0ca5fd1fe2d3f660a1916d243cc24d203a0
normalized_title_hash: e186f55620a2b0df
canonical_url_hash: ''
tags: []
entities:
- Amazon
- Anthropic
- Microsoft
- NVIDIA
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-05-01-pentagon-expands-classified-network-ai-deals-anthropic-notab
related_article_ids:
- 2026-05-01-pentagon-tests-openai-and-google-ai-to-replace-anthropic-in
embedding_id: 2026-05-01-pentagon-signs-ai-deals-with-nvidia-microsoft-aws-excluding
event_name: ''
---

# Pentagon Signs AI Deals with Nvidia, Microsoft, AWS — Excluding Anthropic

The U.S. Department of Defense signed AI deployment agreements with Nvidia, Microsoft, AWS, and Reflection AI for classified IL6/IL7 networks — notably excluding Anthropic, with which it is currently in active litigation. Anthropic had won an injunction in March against the DOD's attempt to brand it a "supply-chain risk" after rejecting the Pentagon's request for unrestricted use of its models for autonomous weapons and mass surveillance. More than 1.3 million DOD personnel now use the secure GenAI.mil platform.

<!-- graph:start -->
## Connections

**Entities:** [[Amazon]] · [[Anthropic]] · [[Microsoft]] · [[NVIDIA]]
**Topics:** [[Model Breakthroughs]]
**Canonical:** [[2026-05-01-pentagon-expands-classified-network-ai-deals-anthropic-notab]]
**Related:** [[2026-05-01-pentagon-tests-openai-and-google-ai-to-replace-anthropic-in]]
<!-- graph:end -->
