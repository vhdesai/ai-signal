---
article_id: 2026-05-22-cohere-releases-command-a-218b-sparse-moe-open-weight-model
title: 'Cohere Releases Command A+: 218B Sparse-MoE Open-Weight Model Under Apache
  2.0'
date: '2026-05-22'
source: Let's Data Science  / Cohere Blog
url_original: https://letsdatascience.com/news/
url_canonical: https://letsdatascience.com/news/
url_status: found
digest_source: digests\raw\2026-05-23_071330_Inbox_Daily AI News Digest – May 23,
  2026.md
content_hash: f2c0af1241fbb9fab5b4b40cd222d12f35d1d3010e43166c491df9043acc2e01
normalized_title_hash: 364948fc4d5dadea
canonical_url_hash: e2fcfb4b9c0b8df1
tags:
- Hot
- New
entities:
- Anthropic
- Cohere
- OpenAI
themes:
- model-capabilities
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-23-cohere-s-command-a-is-a-218-billion-parameter-sparse-mixture
- 2026-05-21-cohere-ships-command-a-218b-sparse-moe-for-agentic-workloads
- 2026-05-20-cohere-ships-command-a-first-apache-2-0-open-model-with-loss
- 2026-05-23-cohere-releases-command-a-218b-sparse-moe-model-for-agentic
- 2026-03-28-cohere-launched-cohere-transcribe-an-open-source-automatic-s
embedding_id: 2026-05-22-cohere-releases-command-a-218b-sparse-moe-open-weight-model
event_name: ''
---

# Cohere Releases Command A+: 218B Sparse-MoE Open-Weight Model Under Apache 2.0

Cohere released Command A+, a 218 billion parameter sparse mixture-of-experts model under the permissive Apache 2.0 open-source license, with a 128,000-token context window. At 218B parameters it is one of the largest commercially open-weight models ever released, designed specifically for enterprise retrieval-augmented generation and multi-step agent workflows. The Apache 2.0 licensing makes it freely usable in commercial products without royalties, directly challenging proprietary offerings from OpenAI and Anthropic for enterprise deployments.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Cohere]] · [[OpenAI]]
**Topics:** [[Model Breakthroughs]] · [[Corporate Moves]]
**Related:** [[2026-05-23-cohere-s-command-a-is-a-218-billion-parameter-sparse-mixture]] · [[2026-05-21-cohere-ships-command-a-218b-sparse-moe-for-agentic-workloads]] · [[2026-05-20-cohere-ships-command-a-first-apache-2-0-open-model-with-loss]] · [[2026-05-23-cohere-releases-command-a-218b-sparse-moe-model-for-agentic]] · [[2026-03-28-cohere-launched-cohere-transcribe-an-open-source-automatic-s]]
<!-- graph:end -->
