---
article_id: 2026-05-21-openai-reasoning-model-disproves-erd-s-unit-distance-conject
title: OpenAI Reasoning Model Disproves Erdős Unit-Distance Conjecture
date: '2026-05-21'
source: OpenAI / Quanta
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-27_071821_Inbox_Daily AI News Digest – May 27,
  2026.md
content_hash: a2fb8b4ffbc85af6b496580beb700e99f36c6373e0dbeea85fa3738349d34550
normalized_title_hash: 4d5bc895ee804096
canonical_url_hash: ''
tags:
- Hot
entities:
- OpenAI
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-05-21-openai-model-disproves-erd-s-unit-distance-conjecture-overtu
related_article_ids:
- 2026-05-25-openai-reasoning-model-disproves-an-80-year-old-erd-s-geomet
- 2026-05-28-openai-reasoning-model-disproves-an-80-year-old-erd-s-conjec
embedding_id: 2026-05-21-openai-reasoning-model-disproves-erd-s-unit-distance-conject
event_name: ''
---

# OpenAI Reasoning Model Disproves Erdős Unit-Distance Conjecture

An OpenAI reasoning model produced a counterexample disproving a long-standing form of the Erdős unit-distance conjecture, with the result independently verified by working combinatorialists. The episode is being treated as a credible data point that frontier reasoning systems can now contribute novel results at the frontier of pure mathematics — not just reproduce known proofs — and is reframing the conversation about AI's role in fundamental research.

<!-- graph:start -->
## Connections

**Entities:** [[OpenAI]]
**Topics:** [[Model Breakthroughs]]
**Canonical:** [[2026-05-21-openai-model-disproves-erd-s-unit-distance-conjecture-overtu]]
**Related:** [[2026-05-25-openai-reasoning-model-disproves-an-80-year-old-erd-s-geomet]] · [[2026-05-28-openai-reasoning-model-disproves-an-80-year-old-erd-s-conjec]]
<!-- graph:end -->
