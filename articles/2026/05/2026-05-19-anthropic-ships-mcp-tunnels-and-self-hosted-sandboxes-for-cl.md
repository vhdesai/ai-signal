---
article_id: 2026-05-19-anthropic-ships-mcp-tunnels-and-self-hosted-sandboxes-for-cl
title: Anthropic ships MCP tunnels and self-hosted sandboxes for Claude Managed Agents
date: '2026-05-19'
source: 9to5Mac
url_original: https://9to5mac.com/2026/05/19/anthropic-enhances-claude-managed-agents-with-two-new-privacy-and-security-features/
url_canonical: https://9to5mac.com/2026/05/19/anthropic-enhances-claude-managed-agents-with-two-new-privacy-and-security-features/
url_status: found
digest_source: digests\raw\2026-05-20_071850_Inbox_Daily AI News Digest – May 20,
  2026.md
content_hash: 27ce62720fb9687f195412c5693c05baef49926cdd6285c76f8ecba3648f791e
normalized_title_hash: b1002df326d8d162
canonical_url_hash: 031c4e9107217c7e
tags:
- New
entities:
- Anthropic
- Google
themes:
- datacenter-infrastructure
cross_cutting_topics: []
dedupe_status: duplicate
canonical_article_id: 2026-05-19-anthropic-adds-enterprise-controls-for-managed-claude-agents
related_article_ids: []
embedding_id: 2026-05-19-anthropic-ships-mcp-tunnels-and-self-hosted-sandboxes-for-cl
event_name: ''
---

# Anthropic ships MCP tunnels and self-hosted sandboxes for Claude Managed Agents

Anthropic shipped two enterprise-grade features for Claude Managed Agents: MCP tunnels (private network routing to internal MCP servers without internet exposure) and self-hosted sandboxes (agent execution environments running inside the customer's own infrastructure under their security controls). The features directly address the top enterprise objections to running agents on third-party clouds, and land on the same day as Google's Gemini Spark launch — underscoring the accelerating race for enterprise-agent market share.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Google]]
**Topics:** [[Infrastructure & Compute]]
**Canonical:** [[2026-05-19-anthropic-adds-enterprise-controls-for-managed-claude-agents]]
<!-- graph:end -->
