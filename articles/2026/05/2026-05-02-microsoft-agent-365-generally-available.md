---
article_id: 2026-05-02-microsoft-agent-365-generally-available
title: Microsoft Agent 365 Generally Available
date: '2026-05-02'
source: Microsoft / AIToolsRecap
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-15_071213_Inbox_Daily AI News Digest – May 15,
  2026.md
content_hash: b79fc0c6ec52cfa2c0ebfdcbf1c8543871817f5e6795e4c04cd0c066d921149b
normalized_title_hash: 40a89f8b21a7f61e
canonical_url_hash: ''
tags: []
entities:
- Anthropic
- Microsoft
- OpenAI
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-02-microsoft-agent-365-goes-generally-available-enterprise-ai-a
- 2026-05-18-microsoft-agent-365-reached-general-availability-on-may-2-ex
- 2026-05-14-microsoft-agent-365-became-generally-available-on-may-2-exte
- 2026-05-18-microsoft-agent-365-generally-available-anthropic-claude-age
- 2026-05-05-microsoft-s-agent-365-a-platform-for-discovering-governing-a
embedding_id: 2026-05-02-microsoft-agent-365-generally-available
event_name: ''
---

# Microsoft Agent 365 Generally Available

Microsoft Agent 365 became generally available on May 2, extending enterprise identity, security, and governance tooling to AI agents across Microsoft 365 environments. The platform enables organizations to deploy, manage, and govern AI agents with the same compliance controls applied to human workers. It represents Microsoft's answer to the enterprise agentic orchestration layer, directly competing with Anthropic's Claude Agent SDK and OpenAI's Workspace Agents for Teams. 💼 Industry News & Deals

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Microsoft]] · [[OpenAI]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-05-02-microsoft-agent-365-goes-generally-available-enterprise-ai-a]] · [[2026-05-18-microsoft-agent-365-reached-general-availability-on-may-2-ex]] · [[2026-05-14-microsoft-agent-365-became-generally-available-on-may-2-exte]] · [[2026-05-18-microsoft-agent-365-generally-available-anthropic-claude-age]] · [[2026-05-05-microsoft-s-agent-365-a-platform-for-discovering-governing-a]]
<!-- graph:end -->
