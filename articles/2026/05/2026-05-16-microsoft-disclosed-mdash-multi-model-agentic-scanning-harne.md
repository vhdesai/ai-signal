---
article_id: 2026-05-16-microsoft-disclosed-mdash-multi-model-agentic-scanning-harne
title: Microsoft disclosed MDASH (Multi-Model Agentic Scanning Harness), a system
  using 100+ specialized AI agents working in…
date: '2026-05-16'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-16_070759_Inbox_Daily AI News Digest – May 16,
  2026.md
content_hash: 420ed7569d7e2d2f51e15c36919f7ee2a48e569e3bfb2c9f86099bac69fe0910
normalized_title_hash: 0226055b23959a9d
canonical_url_hash: ''
tags: []
entities:
- Anthropic
- Google
- Microsoft
- OpenAI
themes:
- policy-regulation
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-12-microsoft-mdash-tops-cybergym-vulnerability-benchmark-at-88
- event-microsoft-build-2026-microsoft-build-2026-security-trust-governance-and-responsib
- 2026-07-27-microsoft-unveils-project-perception-and-an-in-house-mai-cyb
- 2026-07-31-microsoft-introduces-mai-cyber-1-flash-and-project-perceptio
- 2026-07-27-microsoft-launches-first-cybersecurity-model-and-agentic-sec
embedding_id: 2026-05-16-microsoft-disclosed-mdash-multi-model-agentic-scanning-harne
event_name: ''
---

# Microsoft disclosed MDASH (Multi-Model Agentic Scanning Harness), a system using 100+ specialized AI agents working in…

Microsoft disclosed MDASH (Multi-Model Agentic Scanning Harness), a system using 100+ specialized AI agents working in parallel to find real-world software vulnerabilities. MDASH scored 88.45% on the CyberGym benchmark, surpassing single-model systems from both Anthropic and OpenAI. Alongside the disclosure, Microsoft revealed 16 new Windows vulnerabilities discovered by the system — including four critical remote code execution flaws patched in this month's Patch Tuesday. The bet: orchestrated multi-agent systems can outpace any single frontier model on specialized security tasks.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]] · [[Google]] · [[Microsoft]] · [[OpenAI]]
**Topics:** [[Policy & Regulation]] · [[Model Breakthroughs]]
**Related:** [[2026-05-12-microsoft-mdash-tops-cybergym-vulnerability-benchmark-at-88]] · [[event-microsoft-build-2026-microsoft-build-2026-security-trust-governance-and-responsib]] · [[2026-07-27-microsoft-unveils-project-perception-and-an-in-house-mai-cyb]] · [[2026-07-31-microsoft-introduces-mai-cyber-1-flash-and-project-perceptio]] · [[2026-07-27-microsoft-launches-first-cybersecurity-model-and-agentic-sec]]
<!-- graph:end -->
