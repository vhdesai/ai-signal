---
article_id: 2026-05-08-openai-on-may-7-released-a-new-suite-of-real-time-audio-mode
title: 'OpenAI on May 7 released a new suite of real-time audio models for developers:
  GPT-Realtime-2 (the first voice model…'
date: '2026-05-08'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-08_072932_Inbox_Daily AI News Digest – May 8, 2026.md
content_hash: 1bd571c7959d2c3760a1dfc8ad81780d4f60082beeb1dd396add46b0d0bde695
normalized_title_hash: ee78c2a9fd758440
canonical_url_hash: ''
tags: []
entities:
- OpenAI
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-18-openai-released-three-new-voice-api-models-designed-for-live
- 2026-07-06-openai-releases-gpt-realtime-2-1-and-gpt-realtime-2-1-mini-v
- 2026-07-06-openai-ships-gpt-realtime-2-1-and-gpt-realtime-2-1-mini-on-t
- 2026-07-08-openai-launches-gpt-live-full-duplex-voice-models-for-chatgp
- 2026-08-06-openai-details-engineering-behind-its-real-time-voice-system
embedding_id: 2026-05-08-openai-on-may-7-released-a-new-suite-of-real-time-audio-mode
event_name: ''
---

# OpenAI on May 7 released a new suite of real-time audio models for developers: GPT-Realtime-2 (the first voice model…

OpenAI on May 7 released a new suite of real-time audio models for developers: GPT-Realtime-2 (the first voice model with GPT-5-class reasoning, featuring a 128K context window and parallel tool calls); GPT-Realtime-Translate (live speech translation across 70+ input languages into 13 output languages); and GPT-Realtime-Whisper (streaming speech-to-text that transcribes live as the speaker talks). Production partners already deploying the suite include Zillow (voice home-search), Priceline (full trip management by voice), and Deutsche Telekom (multilingual customer support). The models represent a significant shift from call-and-response voice to voice agents that can reason, take action, and carry conversations across languages in real time.

<!-- graph:start -->
## Connections

**Entities:** [[OpenAI]]
**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-05-18-openai-released-three-new-voice-api-models-designed-for-live]] · [[2026-07-06-openai-releases-gpt-realtime-2-1-and-gpt-realtime-2-1-mini-v]] · [[2026-07-06-openai-ships-gpt-realtime-2-1-and-gpt-realtime-2-1-mini-on-t]] · [[2026-07-08-openai-launches-gpt-live-full-duplex-voice-models-for-chatgp]] · [[2026-08-06-openai-details-engineering-behind-its-real-time-voice-system]]
<!-- graph:end -->
