---
article_id: 2026-05-06-anthropic-claude-code-auto-mode-agent-sdk-opens-to-all-devel
title: Anthropic Claude Code Auto Mode + Agent SDK Opens to All Developers
date: '2026-05-06'
source: Anthropic / AIToolsRecap
url_original: null
url_canonical: https://code.claude.com/docs/en/agent-sdk/overview
url_status: repaired
digest_source: digests\raw\2026-05-15_071213_Inbox_Daily AI News Digest – May 15,
  2026.md
content_hash: 23d2415a14a6633aea1da641ae364170457952ee3911c291c51fa44aaf7a6de6
normalized_title_hash: 5f015b46d9e36589
canonical_url_hash: ''
tags: []
entities:
- Anthropic
themes:
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-06-
- 2026-05-14-anthropic-debuts-claude-for-small-business-with-pre-built-ag
- 2026-05-14-anthropic-acknowledges-claude-code-quality-regression-rolls
- 2026-05-12-anthropic-ships-claude-code-agent-view-with-goal-loop-schedu
- 2026-06-26-anthropic-says-claude-now-writes-65-of-its-product-team-s-co
embedding_id: 2026-05-06-anthropic-claude-code-auto-mode-agent-sdk-opens-to-all-devel
event_name: ''
---

# Anthropic Claude Code Auto Mode + Agent SDK Opens to All Developers

Anthropic shipped Claude Code Auto Mode and simultaneously opened the Claude Agent SDK to all external developers — a pivotal week the company described as AI's biggest single week of 2026. Claude Code now accounts for an estimated 4% of all GitHub public commits worldwide, double the figure from just one month prior. The Agent SDK opening, combined with ten financial-services agents launched with JPMorgan, signals Anthropic's aggressive move into the enterprise agentic platform space.

<!-- graph:start -->
## Connections

**Entities:** [[Anthropic]]
**Topics:** [[Corporate Moves]]
**Related:** [[2026-05-06-]] · [[2026-05-14-anthropic-debuts-claude-for-small-business-with-pre-built-ag]] · [[2026-05-14-anthropic-acknowledges-claude-code-quality-regression-rolls]] · [[2026-05-12-anthropic-ships-claude-code-agent-view-with-goal-loop-schedu]] · [[2026-06-26-anthropic-says-claude-now-writes-65-of-its-product-team-s-co]]
<!-- graph:end -->
