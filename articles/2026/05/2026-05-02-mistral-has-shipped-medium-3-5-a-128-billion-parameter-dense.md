---
article_id: 2026-05-02-mistral-has-shipped-medium-3-5-a-128-billion-parameter-dense
title: Mistral has shipped Medium 3.5, a 128-billion-parameter dense merged model
  released under open weights
date: '2026-05-02'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-02_071601_Inbox_Daily AI News Digest – May 2, 2026.md
content_hash: 2f8ecaf62fe741f9a4e971790d690caf79705e8d72d5028e09ac4f2e48e69df0
normalized_title_hash: 220451fcd0ef5e47
canonical_url_hash: ''
tags: []
entities:
- Mistral
themes:
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-05-newmistral-medium-3-5-one-model-three-jobs-half-the-price
- 2026-05-09-mistral-released-medium-3-5-128b-dense-256k-context-window-7
- 2026-04-29-mistral-medium-3-5-released-as-open-source-with-256k-context
- 2026-05-10-mistral-medium-3-5-128b-enterprise-open-weight-model-with-re
- 2026-05-04-mistral-ships-medium-3-5-with-vibe-remote-agents-and-le-chat
embedding_id: 2026-05-02-mistral-has-shipped-medium-3-5-a-128-billion-parameter-dense
event_name: ''
---

# Mistral has shipped Medium 3.5, a 128-billion-parameter dense merged model released under open weights

Mistral has shipped Medium 3.5, a 128-billion-parameter dense merged model released under open weights. The model consolidates chat, multi-step reasoning, and code generation into a single architecture, challenging proprietary offerings in the mid-tier frontier segment. Mistral is positioning Medium 3.5 as a practical enterprise choice for organizations that want frontier-grade capability with on-premise deployment flexibility. Initial benchmarks show strong performance on MMLU, HumanEval, and MATH relative to model size.

<!-- graph:start -->
## Connections

**Entities:** [[Mistral]]
**Topics:** [[Model Breakthroughs]]
**Related:** [[2026-05-05-newmistral-medium-3-5-one-model-three-jobs-half-the-price]] · [[2026-05-09-mistral-released-medium-3-5-128b-dense-256k-context-window-7]] · [[2026-04-29-mistral-medium-3-5-released-as-open-source-with-256k-context]] · [[2026-05-10-mistral-medium-3-5-128b-enterprise-open-weight-model-with-re]] · [[2026-05-04-mistral-ships-medium-3-5-with-vibe-remote-agents-and-le-chat]]
<!-- graph:end -->
