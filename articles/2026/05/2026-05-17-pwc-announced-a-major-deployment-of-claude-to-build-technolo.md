---
article_id: 2026-05-17-pwc-announced-a-major-deployment-of-claude-to-build-technolo
title: PwC announced a major deployment of Claude to build technology, execute deals,
  and reinvent enterprise functions for…
date: '2026-05-17'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-17_070833_Inbox_Daily AI News Digest – May 17,
  2026.md
content_hash: 96a7a5e3c3a2647c7a06b82bc2ca6725b1c8ba5b55909279bf0caddf2998fe30
normalized_title_hash: 7e4d26c059a48588
canonical_url_hash: ''
tags: []
entities:
- IBM
themes: []
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-25-ibm-launches-bob-an-ai-platform-to-govern-sdlc-costs
- 2026-05-14-anthropic-launches-claude-for-small-business-and-expanded-pw
- 2026-05-13-sap-launches-single-enterprise-ai-platform-deepens-ties-with
- 2026-05-18-anthropic-and-pwc-announced-an-expanded-strategic-alliance-i
- 2026-05-03-google-gemini-ai-assistant-deployed-in-millions-of-vehicles
embedding_id: 2026-05-17-pwc-announced-a-major-deployment-of-claude-to-build-technolo
event_name: ''
---

# PwC announced a major deployment of Claude to build technology, execute deals, and reinvent enterprise functions for…

PwC announced a major deployment of Claude to build technology, execute deals, and reinvent enterprise functions for clients — one of the largest professional services AI commitments to date. Separately, IBM launched an AI platform called "Bob" designed to regulate SDLC (software development lifecycle) costs, reflecting IBM's ongoing push to embed AI into enterprise software engineering operations (AI News).

<!-- graph:start -->
## Connections

**Entities:** [[IBM]]
**Related:** [[2026-05-25-ibm-launches-bob-an-ai-platform-to-govern-sdlc-costs]] · [[2026-05-14-anthropic-launches-claude-for-small-business-and-expanded-pw]] · [[2026-05-13-sap-launches-single-enterprise-ai-platform-deepens-ties-with]] · [[2026-05-18-anthropic-and-pwc-announced-an-expanded-strategic-alliance-i]] · [[2026-05-03-google-gemini-ai-assistant-deployed-in-millions-of-vehicles]]
<!-- graph:end -->
