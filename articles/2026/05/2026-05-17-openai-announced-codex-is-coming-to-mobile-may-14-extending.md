---
article_id: 2026-05-17-openai-announced-codex-is-coming-to-mobile-may-14-extending
title: OpenAI announced Codex is coming to mobile (May 14), extending its agentic
  coding platform to phones
date: '2026-05-17'
source: Daily AI News Digest
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-17_070833_Inbox_Daily AI News Digest – May 17,
  2026.md
content_hash: d2cbcf09e680417f0a6a079a9ab3c5bef26ca65a0d9b08a28fccaf62d799855b
normalized_title_hash: 50258f75e14c7613
canonical_url_hash: ''
tags: []
entities:
- Amazon
- Databricks
- OpenAI
themes:
- datacenter-infrastructure
- model-capabilities
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-15-openai-announced-that-its-ai-powered-coding-assistant-codex
- 2026-05-14-openai-brings-codex-to-mobile-extending-agentic-coding-beyon
- 2026-05-14-openai-codex-work-from-anywhere-expansion
- 2026-06-02-openai-extends-codex-to-business-with-plugins-sites-and-anno
- 2026-06-02-openai-extends-codex-beyond-coding-to-finance-banking-and-sa
embedding_id: 2026-05-17-openai-announced-codex-is-coming-to-mobile-may-14-extending
event_name: ''
---

# OpenAI announced Codex is coming to mobile (May 14), extending its agentic coding platform to phones

OpenAI announced Codex is coming to mobile (May 14), extending its agentic coding platform to phones. Amazon launched an AI shopping assistant for the search bar powered by Alexa+ (May 13). Notion converted its workspace into a hub for AI agents the same week. Databricks also announced it is integrating GPT-5.5 into enterprise agent workflows (May 15). The pattern is consistent: frontier models are moving from chat to action-oriented agent interfaces embedded in existing workflows.

<!-- graph:start -->
## Connections

**Entities:** [[Amazon]] · [[Databricks]] · [[OpenAI]]
**Topics:** [[Infrastructure & Compute]] · [[Model Breakthroughs]]
**Related:** [[2026-05-15-openai-announced-that-its-ai-powered-coding-assistant-codex]] · [[2026-05-14-openai-brings-codex-to-mobile-extending-agentic-coding-beyon]] · [[2026-05-14-openai-codex-work-from-anywhere-expansion]] · [[2026-06-02-openai-extends-codex-to-business-with-plugins-sites-and-anno]] · [[2026-06-02-openai-extends-codex-beyond-coding-to-finance-banking-and-sa]]
<!-- graph:end -->
