---
article_id: 2026-05-05-subquadratic-ai-raises-29m-seed-for-subq-12m-token-context-w
title: Subquadratic AI Raises $29M Seed for SubQ — 12M-Token Context with Subquadratic
  Sparse Attention New
date: '2026-05-05'
source: FutureAGI
url_original: null
url_canonical: null
url_status: missing
digest_source: digests\raw\2026-05-12_070743_Inbox_Daily AI News Digest – May 12,
  2026.md
content_hash: b3bd81ad3f347bce584bdf8ff97a3d8dd354d7b141913ee28f8cc517a9eaf2f5
normalized_title_hash: 2bb26a904178212a
canonical_url_hash: ''
tags: []
entities: []
themes:
- model-capabilities
- company-storylines
cross_cutting_topics: []
dedupe_status: canonical
canonical_article_id: null
related_article_ids:
- 2026-05-05-subq-launches-first-commercial-subquadratic-llm-with-12m-tok
- 2026-05-05-trending-subquadratic-claims-1-000x-ai-efficiency-gain-resea
- 2026-04-13-stanford-ai-index-world-ai-compute-grows-3-3-per-year-traini
- 2026-05-10-new-arxiv-may-2026-1-200-ai-papers-agentic-reputation-system
- 2026-04-12-mit-csail-published-research-demonstrating-sparse-activation
embedding_id: 2026-05-05-subquadratic-ai-raises-29m-seed-for-subq-12m-token-context-w
event_name: ''
---

# Subquadratic AI Raises $29M Seed for SubQ — 12M-Token Context with Subquadratic Sparse Attention New

Startup Subquadratic launched on May 5 with $29 million in seed funding to develop SubQ, an LLM using subquadratic sparse attention that delivers a 12-million-token context window. Standard transformer attention scales as O(n²) with sequence length — subquadratic attention is considered the architectural prerequisite for real long-horizon autonomous agents. Whether SubQ's architecture holds under contamination-resistant evaluation will be among the most closely watched model stories heading into June. The seed size and architectural thesis signal serious institutional conviction in moving beyond the transformer bottleneck. 🛠️ 3 — Products & Tools

<!-- graph:start -->
## Connections

**Topics:** [[Model Breakthroughs]] · [[Corporate Moves]]
**Related:** [[2026-05-05-subq-launches-first-commercial-subquadratic-llm-with-12m-tok]] · [[2026-05-05-trending-subquadratic-claims-1-000x-ai-efficiency-gain-resea]] · [[2026-04-13-stanford-ai-index-world-ai-compute-grows-3-3-per-year-traini]] · [[2026-05-10-new-arxiv-may-2026-1-200-ai-papers-agentic-reputation-system]] · [[2026-04-12-mit-csail-published-research-demonstrating-sparse-activation]]
<!-- graph:end -->
